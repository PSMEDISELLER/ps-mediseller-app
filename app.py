...
if "last_created_agent_user" not in st.session_state:
  st.session_state["last_created_agent_user"] = None
if "last_created_agent_name" not in st.session_state:
  st.session_state["last_created_agent_name"] = None
...
# =========================================================
# 9. ADMIN: SETTINGS & AGENTS MANAGEMENT (WITH DIRECT LOGIN & WHATSAPP SHARING)
# =========================================================
  ...
  with set_tab1:
      ...
      if submit_new_ag:
        if new_uname.strip() and new_pass.strip() and new_fname.strip():
          try:
            ...
            conn.commit()
            st.session_state["last_created_agent_user"] = new_uname.strip()
            st.session_state["last_created_agent_name"] = new_fname.strip()
    ...
    # Display generated direct link and WhatsApp share if an agent was just created
    if st.session_state.get("last_created_agent_user"):
      ...
      st.markdown("---")
      st.write("#### 🔗 Direct Login Link (সরাসরি লগইন লিঙ্ক)")
      ...
      st.markdown(
          ...
          <a href="https://wa.me/?text={whatsapp_msg}" target="_blank">
              <button style="background: #25d366; color: white; border: none; padding: 10px 20px; border-radius: 8px; font-weight: bold; cursor: pointer; margin-top: 10px; box-shadow: 0 4px 10px rgba(37,211,102,0.3);">
