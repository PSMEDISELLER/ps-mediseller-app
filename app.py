...
cols_task = [row[1] for row in c.fetchall()]
if "sale_amount" not in cols_task:
    c.execute("ALTER TABLE task_assignments ADD COLUMN sale_amount TEXT DEFAULT '0'")
if "payment_collected_actual" not in cols_task:
...
# Auto cleanup recycle bin older than 3 days
try:
    c.execute("SELECT id, deleted_at FROM recycle_bin")
    for r_bin in c.fetchall():
        d_time = datetime.strptime(r_bin[1], "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone(timedelta(hours=5, minutes=30)))
        if (get_ist_time() - d_time) > timedelta(days=3):
            c.execute("DELETE FROM recycle_bin WHERE id=?", (r_bin[0],))
    conn.commit()
except:
    pass
...
def generate_html_report(title, df):
    ...
    <head>
        ...
        <style>
            ...
            th, td {{ border: 1px solid #e2e8f0; padding: 12px 15px; text-align: left; font-size: 13px; word-break: break-word; }}
...
if st.session_state["user_role"] == "admin":
    menu_options.extend([
        ...
        "⚙️ Settings & Agents (সেটিংস ও এজেন্ট)"
    ])
...
# =========================================================
# 2. SEARCH PARTY DETAILS & ADMIN DELETE OPTION (RECYCLE BIN)
# =========================================================
            ...
            for index, row in doc_df.iterrows():
                ...
                if st.session_state["user_role"] == "admin":
                    if cols[4].button("🗑️ Delete (ডিলিট)", key=f"del_doc_search_{row['id']}"):
                        rec_data = json.dumps(dict(row))
                        c.execute("INSERT INTO recycle_bin (table_name, record_id, record_data, deleted_at) VALUES (?, ?, ?, ?)",
                                  ("locations", row['id'], rec_data, get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
            ...
            for index, row in orders_df.iterrows():
                ...
                if cols[3].button("✔️ Complete (কমপ্লিট)", key=f"ord_btn_{row['id']}"):
                    try:
                        ...
                        err_msg = str(e)
                        c.execute("INSERT INTO error_logs (error_message, log_time) VALUES (?, ?)", (err_msg, get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
                        conn.commit()
                        st.error(f"Error occurred: {err_msg}")
                st.write("---")
        else:
    ...
    if ord_tab2 is not None:
        with ord_tab2:
            ...
            if not completed_ord_df.empty:
                if st.session_state["user_role"] == "admin":
                    export_comp = completed_ord_df[['party_name', 'order_details', 'order_date']].rename(columns={'party_name': 'Party Name', 'order_details': 'Order Details', 'order_date': 'Order Date'})
                    html_comp_ord = generate_html_report("Completed Orders History", export_comp)
                    ...
                    with col_dc2:
                        if st.button("🗑️ Clear All Completed Orders History (সব ডিলিট)", type="secondary"):
                            for idx, row in completed_ord_df.iterrows():
                                rec_data = json.dumps(dict(row))
                                c.execute("INSERT INTO recycle_bin (table_name, record_id, record_data, deleted_at) VALUES (?, ?, ?, ?)",
                                          ("orders", row['id'], rec_data, get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
                            c.execute("DELETE FROM orders WHERE status='Completed'")
                            conn.commit()
                            st.success("All completed orders moved to recycle bin! (রিসাইকেল বিনে পাঠানো হয়েছে!)")
                            st.rerun()
                    st.write("---")
                for idx, row in completed_ord_df.iterrows():
                    if st.session_state["user_role"] == "admin":
                ...
                with col_dw2:
                    if st.button("🗑️ Clear All Daily Work Records (সব কাজ মুছুন)", type="secondary"):
                        ...
                        for row in c.fetchall():
                            rec_data = json.dumps({"id": row[0], "party_name": row[1], "activity_type": row[2], "work_date": row[3]})
                            c.execute("INSERT INTO recycle_bin (table_name, record_id, record_data, deleted_at) VALUES (?, ?, ?, ?)",
                                      ("daily_work", row[0], rec_data, get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
                    ...
                    if st.session_state["user_role"] == "admin":
                        if st.button(f"🗑️ Delete Date Data ({formatted_d}) (সব ডিলিট)", key=f"del_date_{d_str}", type="secondary"):
                            for idx, w_row in date_records.iterrows():
                                rec_data = json.dumps(dict(w_row))
                                c.execute("INSERT INTO recycle_bin (table_name, record_id, record_data, deleted_at) VALUES (?, ?, ?, ?)",
                                          ("daily_work", w_row['id'], rec_data, get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
                ...
                if st.session_state["user_role"] == "admin":
                    ...
                    with col_ms2:
                        if st.button(f"🗑️ Delete All Work Records for Month: {selected_month}", type="secondary"):
                            c.execute("SELECT * FROM daily_work WHERE work_date LIKE ?", (f"{selected_month}%",))
                            for row in c.fetchall():
                                rec_data = json.dumps({"id": row[0], "party_name": row[1], "activity_type": row[2], "work_date": row[3]})
                                c.execute("INSERT INTO recycle_bin (table_name, record_id, record_data, deleted_at) VALUES (?, ?, ?, ?)",
                                          ("daily_work", row[0], rec_data, get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
                            ...
                            conn.commit()
                            st.success(f"All records for {selected_month} moved to recycle bin! (রিসাইকেল বিনে পাঠানো হয়েছে!)")
                            st.rerun()
                    st.write("---")
                else:
...
# =========================================================
    ...
    c.execute("SELECT username, fullname FROM users WHERE is_active=1")
                ...
                if not sel_pt.strip():
                    ...
                    if selected_tasks:
                        ...
                        try:
                            ...
                            conn.commit()
                            st.success("Task assigned successfully! (টাস্ক যোগ করা হয়েছে!)")
                            st.rerun()
                        except Exception as e:
                            err_msg = str(e)
                            c.execute("INSERT INTO error_logs (error_message, log_time) VALUES (?, ?)", (err_msg, get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
                        ...
                        st.error("Select at least one task type (Delivery or Due Collection).")

        st.write("---")
        st.write("#### 📋 Active Assigned Tasks")
        if st.session_state["user_role"] == "admin":
            tasks_df = pd.read_sql_query("SELECT * FROM task_assignments WHERE status='Pending' ORDER BY id DESC", conn)
        else:
            tasks_df = pd.read_sql_query("SELECT * FROM task_assignments WHERE status='Pending' AND agent_name=? ORDER BY id DESC", conn, params=(st.session_state["username"],))

        if not tasks_df.empty:
            for idx, row in tasks_df.iterrows():
                agent_display = agent_name_map.get(row['agent_name'], row['agent_name'])
                with st.expander(f"📦 Party: **{row['party_name']}** | Agent: `{agent_display}` | Type: `{row['task_type']}`", expanded=True):
                    with st.form(f"task_edit_form_{row['id']}"):
                        col_te1, col_te2, col_te3 = st.columns(3)
                        with col_te1:
                            edit_sale = st.text_input("Sale Amount (₹) (বিক্রি)", str(row.get('sale_amount', '0')), key=f"sale_amt_{row['id']}")
                        with col_te2:
                            edit_col = st.text_input("Collection (₹) (কালেকশন)", str(row.get('payment_collected_actual', '0')), key=f"col_amt_{row['id']}")
                        with col_te3:
                            edit_due = st.text_input("Due Amount (₹) (ডিউ)", str(row.get('due_amount', '0')), key=f"due_amt_{row['id']}")

                        submitted_task_edit = st.form_submit_button("💾 Save & Complete Task (সেভ ও কমপ্লিট)", type="primary")
                        if submitted_task_edit:
                            try:
                                c.execute("""
                                    UPDATE task_assignments 
                                    SET sale_amount=?, payment_collected_actual=?, due_amount=?, status='Completed' 
                                    WHERE id=?
                                """, (edit_sale, edit_col, edit_due, row['id']))
                                c.execute("UPDATE agent_live_locations SET completed_dues = completed_dues + 1 WHERE username=?", (row['agent_name'],))
                                conn.commit()
                                st.success("Task updated and marked completed successfully! (সফলভাবে আপডেট হয়েছে!)")
                                st.rerun()
                            except Exception as e:
                                err_msg = str(e)
                                c.execute("INSERT INTO error_logs (error_message, log_time) VALUES (?, ?)", (err_msg, get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
            ...
            st.info("No active tasks found. (কোনো চলমান কাজ নেই।)")

    with task_tab2:
        st.write("#### 📊 Agent & Date-wise Summary (এজেন্ট ও তারিখ অনুযায়ী সামারি)")
        summary_tasks_df = pd.read_sql_query("""
            SELECT agent_name, DATE(created_at) as work_date, COUNT(*) as total_tasks, 
                   SUM(CAST(sale_amount AS REAL)) as total_sale, 
                   SUM(CAST(payment_collected_actual AS REAL)) as total_collection, 
                   SUM(CAST(due_amount AS REAL)) as total_due
            FROM task_assignments
            GROUP BY agent_name, DATE(created_at)
            ORDER BY work_date DESC
        """, conn)
        if not summary_tasks_df.empty:
            summary_tasks_df['Agent'] = summary_tasks_df['agent_name'].apply(lambda x: agent_name_map.get(x, x))
            summary_tasks_df['Date'] = summary_tasks_df['work_date'].apply(lambda x: format_date_display(x))
            display_sum = summary_tasks_df[['Date', 'Agent', 'total_tasks', 'total_sale', 'total_collection', 'total_due']].rename(columns={
                'total_tasks': 'Total Tasks', 'total_sale': 'Total Sale (₹)', 'total_collection': 'Total Collection (₹)', 'total_due': 'Total Due (₹)'
            })
            st.dataframe(display_sum, use_container_width=True)
            if st.session_state["user_role"] == "admin":
                html_sum_rep = generate_html_report("Agent Date-wise Summary Report", display_sum)
                st.download_button(
                    label="📥 Download Summary Report (PDF/HTML)",
                    data=html_sum_rep,
                    file_name="mediseller_agent_summary.html",
                    mime="text/html",
                    type="primary"
                )
        else:
            st.info("No task summary available.")

    with task_tab3:
        st.write("#### 📜 Completed Tasks History")
        comp_tasks_df = pd.read_sql_query("SELECT * FROM task_assignments WHERE status='Completed' ORDER BY id DESC", conn)
            ...
            if st.session_state["user_role"] == "admin":
                if st.button("🗑️ Clear All Completed Tasks History", type="secondary"):
                    for idx, row in comp_tasks_df.iterrows():
                        rec_data = json.dumps(dict(row))
                        c.execute("INSERT INTO recycle_bin (table_name, record_id, record_data, deleted_at) VALUES (?, ?, ?, ?)",
                                  ("task_assignments", row['id'], rec_data, get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
                    c.execute("DELETE FROM task_assignments WHERE status='Completed'")
                    conn.commit()
                    st.success("Moved completed tasks to recycle bin!")
                    st.rerun()
                st.write("---")
            for idx, row in comp_tasks_df.iterrows():
                agent_display = agent_name_map.get(row['agent_name'], row['agent_name'])
                cols = st.columns([2, 2, 2, 2, 1.5])
                cols[0].write(f"Party: **{row['party_name']}**")
                cols[1].write(f"Agent: `{agent_display}`")
                cols[2].write(f"Sale: ₹`{row.get('sale_amount', '0')}` | Coll: ₹`{row.get('payment_collected_actual', '0')}`")
                cols[3].write(f"Due: ₹`{row.get('due_amount', '0')}`")
                if st.session_state["user_role"] == "admin":
                    if cols[4].button("🗑️ Delete", key=f"del_comp_task_{row['id']}"):
                        rec_data = json.dumps(dict(row))
                        c.execute("INSERT INTO recycle_bin (table_name, record_id, record_data, deleted_at) VALUES (?, ?, ?, ?)",
                                  ("task_assignments", row['id'], rec_data, get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
...
# =========================================================
    ...
    st.markdown('<div class="main-title">Route Map & Live Tracking (রুট ম্যাপ)</div>', unsafe_allow_html=True)
    c.execute("SELECT party_name, address, lat, lon FROM locations WHERE lat IS NOT NULL AND lon IS NOT NULL")
    locs = c.fetchall()
    if locs:
        avg_lat = sum([r[2] for r in locs]) / len(locs)
        avg_lon = sum([r[3] for r in locs]) / len(locs)
        route_map = folium.Map(location=[avg_lat, avg_lon], zoom_start=13, tiles=None)
        folium.TileLayer(
            tiles="https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}",
            attr="Google Maps Street",
            name="Street View",
        ...
        for r in locs:
            folium.Marker(
                [r[2], r[3]],
                popup=f"<b>{r[0]}</b><br>{r[1]}",
                tooltip=r[0],
                icon=folium.Icon(color="blue", icon="medkit", prefix="fa")
            ).add_to(route_map)

        c.execute("SELECT username, lat, lon, last_updated FROM agent_live_locations WHERE lat IS NOT NULL AND lon IS NOT NULL")
        agents_locs = c.fetchall()
        for ag in agents_locs:
            folium.Marker(
                [ag[1], ag[2]],
                popup=f"<b>Agent: {ag[0]}</b><br>Last Updated: {ag[3]}",
                tooltip=f"Agent: {ag[0]}",
                icon=folium.Icon(color="green", icon="user", prefix="fa")
            ).add_to(route_map)

        folium.LayerControl().add_to(route_map)
        st_folium(route_map, width="100%", height=500, key="route_map_display")
    else:
        st.info("No mapped locations available for route map.")

# =========================================================
# 7. ATTENDANCE
# =========================================================
elif selected_menu == "📅 Attendance (উপস্থিতি)":
    st.markdown('<div class="main-title">Staff Attendance (উপস্থিতি)</div>', unsafe_allow_html=True)
    today_date_str = get_ist_time().strftime("%Y-%m-%d")
    current_time_str = get_ist_time().strftime("%H:%M:%S")

    col_att1, col_att2 = st.columns(2)
    with col_att1:
        if st.button("✅ Give Attendance Today (আজকের হাজিরা দিন)", type="primary"):
            try:
                c.execute("INSERT INTO attendance (username, date, check_time, status) VALUES (?, ?, ?, ?)",
                          (st.session_state["username"], today_date_str, current_time_str, "Present"))
                conn.commit()
                st.success("Attendance recorded successfully! (হাজিরা নেওয়া হয়েছে!)")
                st.rerun()
            except sqlite3.IntegrityError:
                st.warning("Attendance already recorded for today! (আজকে ইতিমধ্যে হাজিরা দেওয়া হয়েছে!)")

    st.write("---")
    st.write("#### 📋 Attendance Records (হাজিরা তালিকা)")
    att_df = pd.read_sql_query("SELECT username AS 'Username', date AS 'Date', check_time AS 'Time', status AS 'Status' FROM attendance ORDER BY date DESC, id DESC", conn)
    if not att_df.empty:
...
# =========================================================
# 8. LIVE TRACKING (ADMIN ONLY)
# =========================================================
elif selected_menu == "📊 Live Tracking (লাইভ ট্র্যাকিং)" and st.session_state["user_role"] == "admin":
    st.markdown('<div class="main-title">Live Agent Tracking (লাইভ ট্র্যাকিং)</div>', unsafe_allow_html=True)
    live_df = pd.read_sql_query("""
        SELECT a.username, u.fullname, a.lat, a.lon, a.last_updated, a.completed_deliveries, a.completed_dues
        FROM agent_live_locations a
        LEFT JOIN users u ON a.username = u.username
    """, conn)
    if not live_df.empty:
        for idx, row in live_df.iterrows():
            name = row['fullname'] if pd.notna(row['fullname']) and row['fullname'] else row['username']
            st.markdown(f"""
            <div class="card">
                <div class="party-title">Agent: {name} (`{row['username']}`)</div>
                <div class="card-text">📍 Coordinates: <b>{row['lat']}, {row['lon']}</b></div>
                <div class="card-text">🕒 Last Updated: <b>{row['last_updated']}</b></div>
                <div class="card-text">📦 Completed Deliveries: <b>{row['completed_deliveries']}</b></div>
                <div class="card-text">💰 Completed Due Collections: <b>{row['completed_dues']}</b></div>
            </div>
            """, unsafe_allow_html=True)
    else:
        st.info("No live agent tracking data available.")

# =========================================================
# 9. SETTINGS & AGENTS (ADMIN ONLY) - ALL REQUESTED FEATURES
# =========================================================
elif selected_menu == "⚙️ Settings & Agents (সেটিংস ও এজেন্ট)" and st.session_state["user_role"] == "admin":
    st.markdown('<div class="main-title">Settings, Agents & System Management (সেটিংস ও ম্যানেজমেন্ট)</div>', unsafe_allow_html=True)
    ...
    set_tab1, set_tab2, set_tab3, set_tab4, set_tab5 = st.tabs([
        ...
        "💾 DB Backup & Restore",
        "🗑️ Recycle Bin & Error Logs"
    ])

    with set_tab1:
        st.write("#### 👥 Agent Management & Member Count")
        c.execute("SELECT username, fullname, phone, role, is_active FROM users")
        all_users_list = c.fetchall()
        active_members_count = sum(1 for u in all_users_list if u[4] == 1)
        st.markdown(f"""
        <div style="background: #1e293b; padding: 15px; border-radius: 10px; border: 1px solid #3b82f6; margin-bottom: 20px;">
            <h3 style="margin:0; color: #38bdf8;">📊 Total Active Group Members (সক্রিয় মেম্বার সংখ্যা): {active_members_count}</h3>
        </div>
        """, unsafe_allow_html=True)

        st.write("#### Add New Agent / Staff")
        with st.form("add_new_agent_form"):
            new_ag_user = st.text_input("Username (ইউজারনেম, যেমন: agent2)")
            new_ag_pass = st.text_input("Password (পাসওয়ার্ড)", type="password")
            new_ag_name = st.text_input("Full Name (পূর্ণ নাম)")
            new_ag_phone = st.text_input("Phone Number (ফোন নম্বর)")
            submitted_new_agent = st.form_submit_button("➕ Add Agent (এজেন্ট যোগ করুন)", type="primary")
            if submitted_new_agent:
                if new_ag_user.strip() and new_ag_pass.strip():
                    try:
                        c.execute("INSERT INTO users (username, password, role, fullname, phone, created_at, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)",
                                  (new_ag_user.strip(), new_ag_pass.strip(), "staff", new_ag_name.strip(), new_ag_phone.strip(), get_ist_time().strftime("%Y-%m-%d %H:%M:%S"), 1))
                        conn.commit()
                        st.success(f"Agent '{new_ag_user}' added successfully! (এজেন্ট সফলভাবে যুক্ত হয়েছে!)")
                        st.rerun()
                    except sqlite3.IntegrityError:
        ...
        st.write("---")
        st.write("#### Existing Users & Deletion Option")
        for u_row in all_users_list:
            u_user, u_name, u_phone, u_role, u_active = u_row
            col_u1, col_u2, col_u3 = st.columns([3, 2, 2])
            col_u1.write(f"**{u_name if u_name else u_user}** (`{u_user}`) - Role: `{u_role}` | Status: `{'Active' if u_active==1 else 'Deleted/Inactive'}`")
            col_u2.write(f"Phone: {u_phone if u_phone else 'N/A'}")
            if u_user != "admin":
                if u_active == 1:
                    if col_u3.button("🗑️ Delete Agent (বাদ দিন)", key=f"del_agent_{u_user}"):
                        c.execute("UPDATE users SET is_active=0 WHERE username=?", (u_user,))
                        conn.commit()
                        st.success(f"Agent {u_user} removed. Once deleted, they cannot log in unless a new link is generated.")
                        st.rerun()
                else:
                    col_u3.write("❌ Removed (Deleted)")
            else:
                col_u3.write("🔒 Admin")
            st.write("---")

    with set_tab2:
        st.write("#### 🔗 WhatsApp Link Generator for Agents")
        st.write("Generate a valid login link with agent name/details and share directly to WhatsApp. Clicking this link on mobile opens the PWA app directly.")

        with st.form("whatsapp_link_gen_form"):
            link_agent_username = st.text_input("Agent Username (ইউজারনেম যেমন delivery বা নতুন এজেন্ট)")
            link_agent_title = st.text_input("Custom Message / Agent Details (যেমন: P.S Mediseller Agent Login)")
            gen_link_submit = st.form_submit_button("✨ Generate WhatsApp Link (লিংক তৈরি করুন)", type="primary")

            if gen_link_submit:
                if link_agent_username.strip():
                    app_base_url = "https://share.streamlit.io"
                    full_link = f"https://api.whatsapp.com/send?text=" + urllib.parse.quote(
                        f"🩺 *P.S MEDISELLER AGENT ACCESS*\n\nHello {link_agent_title},\nClick the valid secure link below to open your app and login instantly:\n\n👉 {app_base_url}/?login={link_agent_username.strip()}"
                    )
                    st.success("Link generated successfully! Click below to send on WhatsApp:")
                    st.markdown(f'<a href="{full_link}" target="_blank"><button style="background:#25D366; color:white; border:none; padding:12px 20px; border-radius:8px; font-weight:bold; cursor:pointer;">💬 Send via WhatsApp (হোয়াটসঅ্যাপে পাঠান)</button></a>', unsafe_allow_html=True)
                else:
                    st.error("Agent username is required.")

    with set_tab3:
        st.write("#### 🔑 Admin Password Change")
        with st.form("admin_password_change_form"):
            old_pass = st.text_input("Current Admin Password (বর্তমান পাসওয়ার্ড)", type="password")
            new_pass = st.text_input("New Admin Password (নতুন পাসওয়ার্ড)", type="password")
            confirm_pass = st.text_input("Confirm New Password (কনফার্ম পাসওয়ার্ড)", type="password")
            submitted_pass_change = st.form_submit_button("🔒 Change Password (পাসওয়ার্ড পরিবর্তন)", type="primary")
            ...
            if submitted_pass_change:
                ...
                if old_pass == adm_pw:
                    if new_pass.strip() and new_pass == confirm_pass:
                        ...
                        conn.commit()
                        st.success("Admin password changed successfully! (পাসওয়ার্ড সফলভাবে পরিবর্তিত হয়েছে!)")
                    else:
    ...
    with set_tab4:
        st.write("#### 💾 Database (DB) Backup & Restore")
        st.write("Download the current SQLite database or upload a backup DB file to restore data.")
        ...
        st.write("---")
        st.write("#### Restore Database from Backup File")
        uploaded_db = st.file_uploader("Upload DB File (.db)", type=["db"])
        if uploaded_db is not None:
            if st.button("🔄 Restore Database (ডেটাবেজ রিস্টোর)", type="primary"):
                with open(DB_FILE, "wb") as f_out:
                    f_out.write(uploaded_db.getbuffer())
                st.success("Database restored successfully! Please refresh the app. (সফলভাবে রিস্টোর হয়েছে!)")
                st.rerun()

    with set_tab5:
        st.write("#### 🗑️ Recycle Bin (Deleted Items - Auto deleted after 3 days)")
        recycle_df = pd.read_sql_query("SELECT id, table_name, record_id, deleted_at FROM recycle_bin ORDER BY id DESC", conn)
        if not recycle_df.empty:
            for idx, r_row in recycle_df.iterrows():
                cols = st.columns([2, 2, 2, 1.5])
                cols[0].write(f"Table: `{r_row['table_name']}` (ID: {r_row['record_id']})")
                cols[1].write(f"Deleted: `{r_row['deleted_at']}`")
                if cols[2].button("♻️ Restore", key=f"restore_rec_{r_row['id']}"):
                    c.execute("SELECT record_data FROM recycle_bin WHERE id=?", (r_row['id'],))
                    data_json = c.fetchone()[0]
                    rec_dict = json.loads(data_json)
                    t_name = r_row['table_name']
                    try:
                        if t_name == "locations":
                            c.execute("INSERT OR IGNORE INTO locations (id, party_name, address, party_phone, lat, lon, route_order) VALUES (?, ?, ?, ?, ?, ?, ?)",
                                      (rec_dict.get('id'), rec_dict.get('party_name'), rec_dict.get('address'), rec_dict.get('party_phone'), rec_dict.get('lat'), rec_dict.get('lon'), rec_dict.get('route_order', 0)))
                        elif t_name == "orders":
                            c.execute("INSERT OR IGNORE INTO orders (id, party_name, order_details, order_date, status, payment_collected) VALUES (?, ?, ?, ?, ?, ?)",
                                      (rec_dict.get('id'), rec_dict.get('party_name'), rec_dict.get('order_details'), rec_dict.get('order_date'), rec_dict.get('status', 'Pending'), rec_dict.get('payment_collected', '0')))
                        elif t_name == "daily_work":
                            c.execute("INSERT OR IGNORE INTO daily_work (id, party_name, activity_type, work_date) VALUES (?, ?, ?, ?)",
                                      (rec_dict.get('id'), rec_dict.get('party_name'), rec_dict.get('activity_type'), rec_dict.get('work_date')))
                        elif t_name == "task_assignments":
                            c.execute("INSERT OR IGNORE INTO task_assignments (id, agent_name, party_name, task_type, due_amount, sale_amount, payment_collected_actual, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                      (rec_dict.get('id'), rec_dict.get('agent_name'), rec_dict.get('party_name'), rec_dict.get('task_type'), rec_dict.get('due_amount'), rec_dict.get('sale_amount'), rec_dict.get('payment_collected_actual'), rec_dict.get('status'), rec_dict.get('created_at')))
                        c.execute("DELETE FROM recycle_bin WHERE id=?", (r_row['id'],))
                        conn.commit()
                        st.success("Item restored successfully! (রিস্টোর হয়েছে!)")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Restore error: {e}")
                if cols[3].button("🔥 Delete Permanent", key=f"perm_del_{r_row['id']}"):
                    c.execute("DELETE FROM recycle_bin WHERE id=?", (r_row['id'],))
                    conn.commit()
        ...
        st.write("---")
        st.write("#### ⚠️ Error Logs (Execution & Error Fixing Logs)")
        err_df = pd.read_sql_query("SELECT id, error_message, log_time FROM error_logs ORDER BY id DESC LIMIT 50", conn)
        if not err_df.empty:
            if st.button("🗑️ Clear Error Logs", type="secondary"):
            ...
            st.success("No error logs recorded! System is running smoothly.")
