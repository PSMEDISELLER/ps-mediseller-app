...
# ইউজার টেবিল চেক ও তৈরি (না থাকলে)
c.execute(
    """
...
# --- অটো-লগইন চেকিং (URL থেকে) ---
if "login" in st.query_params:
  auto_username = st.query_params["login"]

  # ডাটাবেস থেকে ইউজারের তথ্য চেক করা
  c.execute("SELECT role FROM users WHERE username=?", (auto_username,))
  ...
  if user_data:
    ...
    st.error("ভুল লগইন লিঙ্ক! অনুগ্রহ করে এডমিনের সাথে যোগাযোগ করুন।")

# --- মূল লগইন ও রেজিস্ট্রেশন প্যানেল ---
if not st.session_state.get("authenticated", False):
  st.title("💊 P. S MEDISELLER")

  login_tab, register_tab = st.tabs(
      ["🔑 লগইন (Login)", "📝 নতুন ইউজার তৈরি (Register)"]
  )

  # ১. লগইন ট্যাব
  with login_tab:
    st.markdown("### লগইন প্যানেল")
    with st.form("login_form"):
      ...
      if login_btn:
        ...
        if result and result[0] == password:
          ...
          st.rerun() # পেজ রিফ্রেশ করে মূল অ্যাপে ঢোকার জন্য
        else:
          st.error("ইউজারনেম বা পাসওয়ার্ড ভুল!")

  # ২. নতুন ইউজার রেজিস্টার ট্যাব
  with register_tab:
    st.markdown("### নতুন ইউজার তৈরি করুন")
    with st.form("register_form", clear_on_submit=True):
      reg_fullname = st.text_input("পুরো নাম (Full Name)")
      reg_username = st.text_input("ইউজারনেম (Username)")
      reg_phone = st.text_input("ফোন নম্বর (Phone Number)")
      reg_password = st.text_input("পাসওয়ার্ড (Password)", type="password")
      reg_role = st.selectbox("ইউজার রোল (Role)", ["staff", "admin"])

      reg_btn = st.form_submit_button("ইউজার তৈরি করুন", type="primary")

      if reg_btn:
        if (
            reg_fullname.strip()
            and reg_username.strip()
            and reg_password.strip()
        ):
          try:
            c.execute(
                """
                            INSERT INTO users (username, password, role, fullname, phone, created_at, is_active)
                            VALUES (?, ?, ?, ?, ?, datetime('now', '+5 hours', '+30 minutes'), 1)
                            """,
                (
                    reg_username.strip(),
                    reg_password.strip(),
            ...
            conn.commit()
            st.success("নতুন ইউজার সফলভাবে তৈরি করা হয়েছে! এখন লগইন করুন।")
          except sqlite3.IntegrityError:
            st.error("এই ইউজারনেমটি ইতিমধ্যে তৈরি করা আছে!")
        else:
          st.error("অনুগ্রহ করে সব তথ্য সঠিক ভাবে পূরণ করুন।")

  st.stop() # লগইন না হওয়া পর্যন্ত নিচের কোনো কোড রান করবে না

# ==========================================
# লগইন হওয়ার পর মূল অ্যাপের কোড এখান থেকে শুরু
# ==========================================
st.sidebar.success(f"স্বাগতম, {st.session_state['username']}!")
import base64
...
for row_ord in c.fetchall():
  try:
    ...
    if " " in cleaned_date:
      o_time = datetime.strptime(cleaned_date, "%Y-%m-%d %H:%M:%S").replace(
  ...
  try:
    ...
    if t_status == "completed":
      ...
      if " " in cleaned_task_date:
        ...
        t_time = datetime.strptime(cleaned_task_date, "%Y-%m-%d").replace(
            tzinfo=timezone(timedelta(hours=5, minutes=30))
        )
...
if query_params.get("join") == "true":
  ...
  if st.button(
      "⬅️ Already have an account? Login Here (আগে থেকেই একাউন্ট আছে)"
  ):
    st.warning(
        "⚠️ আপনি সরাসরি প্রবেশ করতে পারবেন না! রেজিস্টার্ড এজেন্ট বা সঠিক অ্যাকাউন্ট ছাড়া"
        " এখানে প্রবেশাধিকার সংরক্ষিত। অনুগ্রহ করে নিচের ফর্ম পূরণ করে অথবা সঠিক"
        " ইউজারনেম দিয়ে লগইন করুন।"
    )
    st.stop()
  ...
  with st.form("agent_public_join_form", clear_on_submit=True):
    ...
    j_pass = st.text_input("Create Password (পাসওয়ার্ড দিন)", type="password")
    submit_j = st.form_submit_button(
        "✅ Submit & Join Team (যুক্ত হন)", type="primary"
    )
    ...
    if submit_j:
      if j_fname.strip() and j_uname.strip() and j_pass.strip():
        try:
          ...
          st.markdown(
              f"<script>localStorage.setItem('ps_mediseller_user',"
              f" '{j_uname.strip()}');</script>",
              unsafe_allow_html=True,
          )

          st.success(
              "Registration successful! Logging in... (রেজিস্ট্রেশন সফল"
              " হয়েছে!)"
          )
          st.query_params.clear()
          st.rerun()
          ...
          st.error(
              "Username already exists! Choose another. (এই ইউজারনেম ইতিমধ্যে"
              " আছে!)"
          )
      else:
...
if show_notif and total_pending_items > 0:
  ...
  with col_n1:
    st.warning(
        "🔔 **নোটিফিকেশন:** আপনার অর্ডার পেন্ডিং বা ডিউ পেন্ডিং রয়েছে। **পেন্ডিং Order"
        " খাতায় তুলতে বাকি!**"
    )
  with col_n2:
...
if st.session_state["user_role"] == "admin":
  menu_options.extend(
      ["📊 Live Tracking (লাইভ ট্র্যাকিং)", "⚙️ Settings & Agents (সেটিংসে)"]
  )
...
if selected_menu == "📍 Add Location (লোকেশন যোগ)":
  ...
  if "With Map Party" in selected_entry_tab:
    ...
    if submitted_loc:
      if p_name.strip() and p_phone.strip():
        ...
        if existing_check:
          st.error(
              "Party name or phone already exists! (ইতিমধ্যে সেভ করা আছে!)"
          )
        else:
          try:
  ...
  with col_m2:
    st.write(
        "Coordinates"
        f" (স্থানাঙ্ক): `{st.session_state['selected_lat']:.5f},"
  ...
  if map_data and map_data.get("last_clicked"):
    ...
    if (
        clicked_lat != st.session_state["selected_lat"]
        or clicked_lon != st.session_state["selected_lon"]
    ):
  ...
  order_search_text = st.text_input(
      ...
      key="order_party_search_text_input",
      label_visibility="collapsed",
  )
  if order_search_text.strip():
    q_term = f"%{order_search_text.strip()}%"
    c.execute(
        "SELECT party_name FROM locations WHERE party_name LIKE ? OR address"
        " LIKE ? OR party_phone LIKE ? ORDER BY party_name ASC",
  ...
  if order_search_text.strip() and filtered_parties_list:
    st.markdown(
        f"<p style='color: #60a5fa; font-size: 12px; margin: 2px 0;'>💡"
        f" Suggestions ({len(filtered_parties_list)} found): Select"
        " below</p>",
        unsafe_allow_html=True,
    )
  ...
  with st.form("order_visit_entry_form", clear_on_submit=True):
    ...
    with col_ob1:
      submitted_order = st.form_submit_button(
          "🛒 Submit Order (অর্ডার জমা)", type="primary"
      )
    with col_ob2:
    ...
    if submitted_order:
      if not selected_order_party_native.strip():
        ...
        c.execute(
            ...
            " payment_collected) VALUES (?, ?, ?, ?, ?)",
            (
                selected_order_party_native.strip(),
                ord_details.strip(),
                get_ist_time().strftime("%Y-%m-%d %H:%M:%S"),
                "Pending",
                "0",
  ...
  report_df = pd.read_sql_query(
      "SELECT party_name AS 'Party Name', activity_type AS 'Activity Type',"
      " work_date AS 'Work Date' FROM daily_work ORDER BY work_date DESC, id"
      " DESC LIMIT 20",
      conn,
  )
  if not report_df.empty:
    if st.session_state["user_role"] == "admin":
      full_report_df = pd.read_sql_query(
          "SELECT party_name AS 'Party Name', activity_type AS 'Activity"
          " Type', work_date AS 'Work Date' FROM daily_work ORDER BY work_date"
          " DESC, id DESC",
          conn,
      )
      html_all_report = generate_html_report(
          "Daily Work & Visit Report", full_report_df
      )
      st.download_button(
...
# =========================================================
  ...
  if st.session_state.get("mapping_party_id"):
    ...
    st.write(
        "Click correct location on map and click **'Save Location'** below."
    )
    ...
    with col_tm1:
      if st.button(
          "📍 Current GPS (কারেন্ট জিপিএস)", key="btn_curr_gps_temp"
      ):
        if gps_lat and gps_lon:
    ...
    with col_tm2:
      st.write(
          "Coordinates:"
          f" `{st.session_state['temp_map_lat']:.5f},"
          f" {st.session_state['temp_map_lon']:.5f}`"
      )
    pick_map = folium.Map(
        location=[
    ...
    if p_map_data and p_map_data.get("last_clicked"):
      ...
      if (
          clat != st.session_state["temp_map_lat"]
          or clon != st.session_state["temp_map_lon"]
      ):
    ...
    with col_b1:
      if st.button(
          "✅ Save Location (সেভ করুন)",
          type="primary",
          key="save_party_map_ok",
      ):
  ...
  if master_search_query.strip():
    ...
    df = pd.read_sql_query(
        "SELECT * FROM locations WHERE party_name LIKE ? OR address LIKE ? OR"
        " party_phone LIKE ? ORDER BY party_name ASC",
        conn,
        params=(q_term, q_term, q_term),
    )
  else:
    df = pd.read_sql_query(
        "SELECT * FROM locations ORDER BY party_name ASC", conn
    )

  if st.session_state["user_role"] == "admin" and not df.empty:
    html_locs_df = generate_html_report(
        "Locations & Parties Directory",
        df[["party_name", "address", "party_phone"]].rename(
            columns={
  ...
  with st.expander(
      f"🩺 Non-Map List ({len(doc_df)} Entries) (ম্যাপবিহীন তালিকা)",
      expanded=True,
  ):
    if not doc_df.empty:
      for index, row in doc_df.iterrows():
        ...
        cols[1].write(
            row["party_phone"]
            if row["party_phone"]
            else "No number (নম্বর নেই)"
        )
        cols[2].write(
            row["address"] if row["address"] else "No address (ঠিকানা নেই)"
        )

        if cols[3].button(
            "📍 Add Map (ম্যাপ যুক্ত)", key=f"map_add_search_{row['id']}"
        ):
          st.session_state["mapping_party_id"] = row["id"]
          st.session_state["mapping_party_name"] = row["party_name"]
          st.session_state["temp_map_lat"] = st.session_state.get(
              "selected_lat", 22.8620
          )
          st.session_state["temp_map_lon"] = st.session_state.get(
              "selected_lon", 87.3320
          )
          st.rerun()
        if st.session_state["user_role"] == "admin":
          if cols[4].button(
              "🗑️ Delete (ডিলিট)", key=f"del_doc_search_{row['id']}"
          ):
            move_to_recycle_bin("Location", row["party_name"], dict(row))
            c.execute("DELETE FROM locations WHERE id=?", (row["id"],))
            conn.commit()
            st.success(
                "Moved to Recycle Bin! (রিসাইকেল বিনে পাঠানো হয়েছে!)"
            )
            st.rerun()
        st.write("---")
    else:
  ...
  st.write("---")
  st.write(
      f"#### 📍 Mapped List ({len(mapped_df)} Records) (ম্যাপযুক্ত তালিকা)"
  )
  if not mapped_df.empty:
    for index, row in mapped_df.iterrows():
      ...
      cols[0].write(f"**{row['party_name']}**")
      cols[1].write(
          row["party_phone"] if row["party_phone"] else "No number (নম্বর নেই)"
      )
      cols[2].write(
          row["address"] if row["address"] else "No address (ঠিকানা নেই)"
      )
      ...
      cols[3].markdown(
          ...
          " linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%); color:white;"
          " border:none; padding:6px 12px; border-radius:6px; cursor:pointer;"
          ' font-weight:600;">🧭 Direction (ডিরেকশন)</button></a>',
          unsafe_allow_html=True,
      )
      if st.session_state["user_role"] == "admin":
        if cols[4].button(
            "🗑️ Delete (ডিলিট)", key=f"del_loc_search_{row['id']}"
        ):
          move_to_recycle_bin("Location", row["party_name"], dict(row))
          c.execute("DELETE FROM locations WHERE id=?", (row["id"],))
          conn.commit()
...
# =========================================================
  ...
  if st.session_state["user_role"] == "admin":
    ord_tab1, ord_tab2 = st.tabs([
        "⏳ Pending Orders (পেন্ডিং)",
        "📜 Completed History (সম্পন্ন অর্ডার)",
    ])
  else:
  ...
  with ord_tab1:
    ...
    if st.session_state["user_role"] == "admin":
      all_ord_df = pd.read_sql_query(
          "SELECT party_name AS 'Party Name', order_details AS 'Order Details',"
          " order_date AS 'Order Date' FROM orders WHERE status='Pending'"
          " ORDER BY order_date DESC",
          conn,
      )
      if not all_ord_df.empty:
        html_ord_report = generate_html_report(
            "Pending Orders Report", all_ord_df
        )
        st.download_button(
    ...
    if not orders_df.empty:
      for index, row in orders_df.iterrows():
        ...
        if cols[3].button("✔️ Complete (কমপ্লিট)", key=f"ord_btn_{row['id']}"):
          ...
          conn.commit()
          c.execute(
              "UPDATE agent_live_locations SET completed_deliveries ="
              " completed_deliveries + 1 WHERE username=?",
              (st.session_state["username"],),
          )
          conn.commit()
  ...
  if ord_tab2 is not None:
    with ord_tab2:
      ...
      completed_ord_df = pd.read_sql_query(
      ...
      if not completed_ord_df.empty:
        if st.session_state["user_role"] == "admin":
          html_comp_ord = generate_html_report(
              "Completed Orders History",
              completed_ord_df[["party_name", "order_details", "order_date"]],
          )
          ...
          with col_dc2:
            if st.button(
                "🗑️ Clear All Completed Orders History (সব ডিলিট)",
                type="secondary",
            ):
              ...
              conn.commit()
              st.success(
                  "All completed orders history moved to Recycle Bin!"
                  " (রিসাইকেল বিনে পাঠানো হয়েছে!)"
              )
              st.rerun()
          st.write("---")
...
# =========================================================
  ...
  with work_tab1:
    ...
    if st.session_state["user_role"] == "admin":
      ...
      if not full_dw_df.empty:
        html_dw_report = generate_html_report(
            "Daily Work Report", full_dw_df
        )
        ...
        with col_dw2:
          if st.button(
              "🗑️ Clear All Daily Work Records (সব কাজ মুছুন)",
              type="secondary",
          ):
            ...
            conn.commit()
            st.success(
                "All daily work records moved to Recycle Bin! (রিসাইকেল বিনে"
                " পাঠানো হয়েছে!)"
            )
            st.rerun()
        st.write("---")
    work_df = pd.read_sql_query(
        "SELECT * FROM daily_work ORDER BY work_date DESC, id DESC", conn
    )
    if not work_df.empty:
      ...
      for d_str in unique_dates:
        ...
        with st.expander(
            f"📅 Date: {formatted_d} (Total: {count_parties})", expanded=False
        ):
          if st.session_state["user_role"] == "admin":
            if st.button(
                f"🗑️ Delete Date Data ({formatted_d}) (সব ডিলিট)",
                key=f"del_date_{d_str}",
                type="secondary",
            ):
              for _, w_row in date_records.iterrows():
                move_to_recycle_bin(
                    "Daily Work", w_row["party_name"], dict(w_row)
                )
          ...
          for idx, w_row in date_records.iterrows():
            ...
            if st.session_state["user_role"] == "admin":
              if cols[2].button(
                  "🗑️ Delete (ডিলিট)", key=f"del_dw_{w_row['id']}"
              ):
                move_to_recycle_bin(
                    "Daily Work", w_row["party_name"], dict(w_row)
                )
  ...
  with work_tab2:
    st.write(
        "#### 📊 Monthly Doctor/Party Activity Report (মাসিক ডাক্তার ও পার্টি"
        " রিপোর্ট)"
    )
    ...
    with col_yr:
      selected_year = st.selectbox(
          "Select Year (বছর)", [2026, 2025, 2024], index=0
      )
    with col_mo:
      months_dict = {
          "01": "January (জানুয়ারি)",
          "02": "February (ফেব্রুয়ারি)",
          "03": "March (মার্চ)",
          "04": "April (এপ্রিল)",
          "05": "May (মে)",
          "06": "June (জুন)",
      ...
      selected_mo_key = st.selectbox(
          ...
          index=(
              list(months_dict.keys()).index(current_mo_num)
              if current_mo_num in months_dict
              else 7
          ),
      )
    selected_month = f"{selected_year}-{selected_mo_key}"
    if selected_month.strip():
      all_locs_df = pd.read_sql_query(
          "SELECT party_name, address, lat, lon FROM locations ORDER BY"
          " party_name ASC",
          conn,
      )
      ...
      if not all_locs_df.empty:
        ...
        for idx, loc_row in all_locs_df.iterrows():
          ...
          is_mapped = (
              "Mapped (ম্যাপযুক্ত)"
              if pd.notna(loc_row["lat"]) and pd.notna(loc_row["lon"])
              else "Non-Map (ম্যাপবিহীন)"
          )
        ...
        if st.session_state["user_role"] == "admin":
          html_summary = generate_html_report(
              f"Monthly Summary - {selected_month}", report_summary_df
          )
          ...
          with col_ms2:
            if st.button(
                f"🗑️ Delete All Work Records for Month: {selected_month}",
                type="secondary",
            ):
              c.execute(
                  "DELETE FROM daily_work WHERE work_date LIKE ?",
                  (f"{selected_month}%",),
              )
              conn.commit()
              st.success(
                  f"All records for {selected_month} deleted successfully!"
                  " (মুছে ফেলা হয়েছে!)"
              )
              st.rerun()
          st.write("---")
          ...
          st.markdown(
              "<p style='color: #60a5fa; font-size: 13px;'><i>Note: Monthly"
              " report downloads and management are restricted to admins"
              " only. Agents can only view their summary above.</i></p>",
              unsafe_allow_html=True,
          )

        st.dataframe(report_summary_df, use_container_width=True)
        zero_activity_df = report_summary_df[
            (report_summary_df["Total Visits"] == 0)
            & (report_summary_df["Total Orders"] == 0)
        ]

        st.write(
            "⚠️ **Doctors/Parties with ZERO Visits & ZERO Orders"
            f" ({len(zero_activity_df)}):**"
        )
        if not zero_activity_df.empty:
          st.dataframe(zero_activity_df, use_container_width=True)
        else:
          st.success(
              "All parties/doctors had at least one visit or order this month!"
              " (সব ডাক্তারের ভিজিট বা অর্ডার হয়েছে!)"
          )
      else:
...
# =========================================================
  ...
  st.markdown(
      '<div class="main-title">Delivery & Due Plan (ডেলিভারি ও ডিউ'
      " প্ল্যান)</div>",
      unsafe_allow_html=True,
  )
  ...
  c.execute(
      "SELECT party_name, lat, lon FROM locations ORDER BY party_name ASC"
  )
  loc_data = c.fetchall()
  ...
  with task_tab1:
    if st.session_state["user_role"] == "admin":
      ...
      if not full_tasks_df.empty:
        ...
        export_tasks_df["Agent Name"] = export_tasks_df.apply(
            lambda r: (
                r["agent_fullname"]
                if pd.notna(r["agent_fullname"]) and r["agent_fullname"]
                else r["agent_name"]
            ),
            axis=1,
        )
        ...
        html_tasks_report = generate_html_report(
            "Active Tasks & Deliveries Report", export_tasks_df_final
        )
        st.download_button(
    ...
    task_search_text = st.text_input(
        ...
        placeholder="Type name, address or keyword...",
        key="task_party_search_text_input",
        label_visibility="collapsed",
    )
    ...
    if task_search_text.strip() and filtered_task_parties:
      st.markdown(
          f"<p style='color: #60a5fa; font-size: 12px; margin: 2px 0;'>💡"
          f" Suggestions ({len(filtered_task_parties)} found): Select"
          " below</p>",
          unsafe_allow_html=True,
      )
      sel_pt = st.radio(
    ...
    with st.form("easy_assign_form", clear_on_submit=True):
      ...
      sel_ag = st.selectbox(
          ...
          all_agents,
          index=(
              all_agents.index(current_logged_user)
              if current_logged_user in all_agents
              else 0
          ),
      ...
      submit_easy_task = st.form_submit_button(
          "🎯 Add Task (কাজ যোগ)", type="primary"
      )
      ...
      if submit_easy_task:
        if not sel_pt.strip():
          ...
          if selected_tasks:
            ...
            c.execute(
                "INSERT INTO task_assignments (agent_name, party_name,"
                " task_type, due_amount, status, created_at) VALUES (?, ?, ?,"
                " ?, ?, ?)",
                (
                    sel_ag,
                    sel_pt.strip(),
            ...
            st.error(
                "Please select at least one task type (Delivery or Due"
                " Collection)."
            )
    ...
    if not pending_tasks_df.empty:
      ...
      for ag_username in unique_agents_in_tasks:
        ag_rows = pending_tasks_df[
            pending_tasks_df["agent_name"] == ag_username
        ]
        ag_disp_name = (
            ag_rows.iloc[0]["agent_fullname"]
            if pd.notna(ag_rows.iloc[0]["agent_fullname"])
            else ag_username
        )

        with st.expander(
            f"👤 Agent: {ag_disp_name} ({len(ag_rows)} Pending Tasks)",
            expanded=True,
        ):
          for idx, row in ag_rows.iterrows():
            st.markdown(
                f"**Party:** `{row['party_name']}` | **Task:**"
                f" `{row['task_type']}` | **Assigned Due:** ₹`{row['due_amount']}`"
            )
            if pd.notna(row["address"]) and row["address"]:
              st.markdown(
                  f"📍 Address: {row['address']}"
                  f" {(' | Ph: ' + str(row['party_phone'])) if pd.notna(row['party_phone']) else ''}"
              )

            with st.form(
                key=f"complete_task_form_{row['id']}", clear_on_submit=True
            ):
              col_f1, col_f2 = st.columns(2)
              with col_f1:
                sale_input = st.text_input(
                    "Total Sale Amount (কত টাকা সেল ছিল)",
                    value="0",
                    key=f"sale_amt_{row['id']}",
                )
              with col_f2:
                payment_input = st.text_input(
                    "Payment Collected (কত টাকা পেমেন্ট দিয়েছে)",
                    value=str(row["due_amount"]),
                    key=f"pay_amt_{row['id']}",
                )

              submit_complete = st.form_submit_button(
                  "✔️ Complete Task (সম্পন্ন বলে ক্লিক করুন)", type="primary"
              )
              if submit_complete:
                c.execute(
                    "UPDATE task_assignments SET status='Completed',"
                    " sale_amount=?, payment_collected_actual=? WHERE id=?",
                ...
                conn.commit()
                st.success(
                    "Task marked as completed successfully! (সম্পন্ন হয়েছে!)"
                )
                st.rerun()

            if st.session_state["user_role"] == "admin":
              if st.button(
                  "🗑️ Delete Task (টাস্ক ডিলিট)",
                  key=f"del_pend_task_{row['id']}",
              ):
                move_to_recycle_bin("Task", row["party_name"], dict(row))
                c.execute(
                    "DELETE FROM task_assignments WHERE id=?", (row["id"],)
                )
                conn.commit()
  ...
  with task_tab2:
    ...
    if not agent_sum_df.empty:
      if st.session_state["user_role"] == "admin":
        ...
        export_sum_df["Date"] = export_sum_df["task_date"].apply(
            lambda x: format_date_display(x)
        )
        export_sum_df["Total Tasks"] = export_sum_df["total_tasks"]
        export_sum_df["Completed Tasks"] = export_sum_df["completed_tasks"]

        export_sum_df_final = export_sum_df[[
            "Agent Name",
            "Date",
            "Total Tasks",
            "Completed Tasks",
        ]]
        html_agent_sum = generate_html_report(
            "Agent Task Summary Report", export_sum_df_final
        )
        st.download_button(
      ...
      for idx, row in agent_sum_df.iterrows():
        ag_disp = (
            row["agent_fullname"]
            if pd.notna(row["agent_fullname"]) and row["agent_fullname"]
            else row["agent_name"]
        )
        ...
        if st.session_state["user_role"] == "admin":
          if st.button(
              f"🗑️ Delete Tasks ({ag_disp} - {t_date})",
              key=(
                  f"del_agent_date_sum_{row['agent_name']}_{row['task_date']}"
              ),
          ):
            c.execute(
                "DELETE FROM task_assignments WHERE agent_name=? AND"
                " SUBSTR(created_at, 1, 10)=?",
                (row["agent_name"], row["task_date"]),
            )
            conn.commit()
  ...
  with task_tab3:
    ...
    if not completed_tasks_df.empty:
      if st.session_state["user_role"] == "admin":
        ...
        export_comp_df["Agent Name"] = export_comp_df.apply(
            lambda r: (
                r["agent_fullname"]
                if pd.notna(r["agent_fullname"]) and r["agent_fullname"]
                else r["agent_name"]
        ...
        export_comp_df["Collection Amount (₹)"] = export_comp_df[
            "payment_collected_actual"
        ]
        export_comp_df["Completed Date"] = export_comp_df["created_at"].apply(
            lambda x: format_date_display(x)
        )
        ...
        html_comp_tasks = generate_html_report(
            "Completed Tasks History", export_comp_df_final
        )
      ...
      for idx, row in completed_tasks_df.iterrows():
        ...
        st.markdown(
            f"**Agent:** `{ag_c_name}` | **Party:** `{row['party_name']}` |"
            f" **Task:** `{row['task_type']}`"
        )
        st.markdown(
            f"📦 Sale: ₹`{row['sale_amount']}` | 💰 Collected:"
            f" ₹`{row['payment_collected_actual']}` | ⏳ Due:"
            f" ₹`{row['due_amount']}`"
        )
        if st.session_state["user_role"] == "admin":
...
# =========================================================
  ...
  c.execute(
      "SELECT party_name, address, lat, lon, party_phone FROM locations WHERE"
      " lat IS NOT NULL AND lon IS NOT NULL ORDER BY party_name ASC"
  )
  ...
  if route_data:
    ...
    for idx, r in enumerate(route_data):
      ...
      popup_html = (
          f"<b>{p_n}</b><br>{p_a or ''}<br>{('Ph: ' + str(p_ph)) if p_ph else ''}"
      )
      folium.Marker(
          [p_lat, p_lon],
    ...
    st.info(
        "No mapped locations available to show on route map. (ম্যাপযুক্ত কোনো"
        " লোকেশন নেই।)"
    )
...
# =========================================================
  ...
  with att_tab1:
    ...
    with st.form("attendance_form", clear_on_submit=True):
      ...
      st.write(
          f"Agent: **{agent_name_map.get(agent_for_att, agent_for_att)}**"
      )
      submit_att = st.form_submit_button(
          "✅ Give Attendance / Check-in (উপস্থিতি দিন)", type="primary"
      )
      if submit_att:
        try:
          ...
          conn.commit()
          st.success(
              "Attendance recorded successfully! (উপস্থিতি নথিভুক্ত হয়েছে!)"
          )
          st.rerun()
        except sqlite3.IntegrityError:
          st.warning(
              "Attendance already given for today! (আজকে ইতিমধ্যে উপস্থিতি"
              " দেওয়া হয়েছে!)"
          )

    st.write("---")
    st.write(
        "#### 📋 Today's Attendance List (আজকের উপস্থিতি তালিকা - সবাই দেখবে)"
    )
    ...
    if not today_att_df.empty:
      today_att_df["Date"] = today_att_df["Date"].apply(
          lambda x: format_date_display(x)
      )
  ...
  with att_tab2:
    ...
    if current_role == "admin":
      ...
      st.write(
          "নিচে সব এজেন্টের নামের তালিকা দেওয়া হলো। যেকোনো এজেন্টে ক্লিক বা সিলেক্ট"
          " করলে তার পুরো মাসের তারিখ অনুযায়ী উপস্থিতি দেখতে পাবেন:"
      )
      ...
      if staff_list:
        for s_user, s_fname in staff_list:
          ...
          c.execute(
              "SELECT COUNT(*) FROM attendance WHERE username=?", (s_user,)
          )
          total_att_count = c.fetchone()[0]

          with st.expander(
              f"👤 Agent: {display_name} (`{s_user}`) — Total Attendance:"
              f" {total_att_count}",
              expanded=False,
          ):
            ...
            if not agent_att_df.empty:
              agent_att_df["Date"] = agent_att_df["Date"].apply(
                  lambda x: format_date_display(x)
              )
      ...
      if not all_att_report_df.empty:
        all_att_report_df["Date"] = all_att_report_df["Date"].apply(
            lambda x: format_date_display(x)
        )
        html_all_att = generate_html_report(
            "Complete Attendance Report", all_att_report_df
        )
        st.download_button(
      ...
      if not staff_att_df.empty:
        staff_att_df["Date"] = staff_att_df["Date"].apply(
            lambda x: format_date_display(x)
        )
      ...
      st.markdown(
          "<p style='color: #60a5fa; font-size: 13px; margin-top: 10px;'><i>Note:"
          " Agents can only view their own attendance records. Report downloads"
          " are restricted to admins only.</i></p>",
          unsafe_allow_html=True,
      )
...
# =========================================================
  ...
  st.markdown(
      "<p style='color: #38bdf8; font-size: 13px;'><i>💡 Note: This page"
      " automatically refreshes every 10 seconds to show the latest background"
      " GPS pings from all active agents.</i></p>",
      unsafe_allow_html=True,
  )
  ...
  if not live_df.empty:
    for idx, r in live_df.iterrows():
      name = (
          r["fullname"]
          if pd.notna(r["fullname"]) and r["fullname"]
          else r["username"]
      )
      username = r["username"]
      ...
      if pd.notna(last_up):
        try:
          up_dt = datetime.strptime(
              str(last_up), "%Y-%m-%d %H:%M:%S"
          ).replace(tzinfo=timezone(timedelta(hours=5, minutes=30)))
      ...
      with st.expander(
          f"👤 Agent: {name} (`{username}`) — Last Active: {time_ago_str}",
          expanded=False,
      ):
        st.markdown(
            f"""
        ...
        if pd.notna(lat) and pd.notna(lon):
          google_maps_track_url = (
              f"https://www.google.com/maps/search/?api=1&query={lat},{lon}"
          )
          st.markdown(
              f'<a href="{google_maps_track_url}" target="_blank"'
              ' style="text-decoration:none;"><button style="background:'
              " linear-gradient(135deg, #10b981 0%, #059669 100%); color:white;"
              " border:none; padding:10px 20px; border-radius:8px;"
              " cursor:pointer; font-weight:600; font-size:15px;"
              " margin-top:10px; box-shadow: 0 4px 12px"
              ' rgba(16,185,129,0.4);">🗺️ Track Agent on Google Maps (গুগল ম্যাপে'
              " ট্র্যাক করুন)</button></a>",
              unsafe_allow_html=True,
          )
        else:
...
# =========================================================
    ...
    selected_menu == "⚙️ Settings & Agents (সেটিংসে)"
    and st.session_state["user_role"] == "admin"
):
  ...
  with set_tab1:
    ...
    if eval_parent_url and "component" not in eval_parent_url:
      ...
      clean_base_url = (
          "https://ps-mediseller-app-gcanjbehuut7h9rzk4xzfg.streamlit.app"
      )
    ...
    st.info(
        "💡 এই লিঙ্কটি আপনি আপনার নতুন এজেন্টদের হোয়াটসঅ্যাপ বা মেসেজে কপি করে শেয়ার"
        " করবেন। তাঁরা এই লিঙ্ক খুললে সরাসরি নাম, ফোন নম্বর ও পাসওয়ার্ড দিয়ে একাউন্ট"
        " তৈরি করতে পারবে এবং সাথে সাথে নিচের লিস্টে যুক্ত হবে।"
    )
    ...
    with st.form("add_agent_form", clear_on_submit=True):
      ...
      submit_new_ag = st.form_submit_button(
          "➕ Add Agent (যোগ করুন)", type="primary"
      )
      if submit_new_ag:
    ...
    st.write("---")
    st.write(
        "#### Existing Agents List & Edit Option (কর্মীদের তালিকা ও পরিবর্তন)"
    )
    agents_df = pd.read_sql_query(
        "SELECT username, fullname, phone, password, role, created_at FROM"
    ...
    if not agents_df.empty:
      for idx, row in agents_df.iterrows():
        with st.expander(
            f"👤 {row['fullname']} (`{row['username']}`)", expanded=False
        ):
          st.write(
              f"**Phone:** {row['phone']} | **Joined Date:**"
              f" {format_date_display(row['created_at'])}"
          )
          with st.form(
              key=f"edit_agent_{row['username']}", clear_on_submit=False
          ):
            e_pass = st.text_input(
                "New Password (নতুন পাসওয়ার্ড)", value=row["password"]
            )
            e_phone = st.text_input(
                "New Phone (নতুন ফোন)",
                value=row["phone"] if row["phone"] else "",
            )

            col_ea1, col_ea2 = st.columns(2)
            with col_ea1:
              sub_edit = st.form_submit_button(
                  "💾 Update (আপডেট)", type="primary"
              )
            with col_ea2:
            ...
            if sub_edit:
              c.execute(
                  "UPDATE users SET password=?, phone=? WHERE username=?",
                  (e_pass, e_phone, row["username"]),
              )
              conn.commit()
              st.success("Agent details updated! (আপডেট হয়েছে!)")
              st.rerun()

            if sub_del:
              c.execute(
                  "DELETE FROM users WHERE username=?", (row["username"],)
              )
              conn.commit()
              st.success("Agent deleted! (কর্মী ডিলিট হয়েছে!)")
              st.rerun()
    else:
  ...
  with set_tab3:
    st.write("#### 🗑️ Recycle Bin (মুছে ফেলা তথ্য)")
    bin_df = pd.read_sql_query(
        "SELECT * FROM recycle_bin ORDER BY id DESC", conn
    )
    if not bin_df.empty:
      st.dataframe(bin_df, use_container_width=True)
      if st.button("🗑️ Empty Recycle Bin (সম্পূর্ণ খালি করুন)"):
  ...
  with set_tab4:
    ...
    with st.form("change_admin_pass_form"):
      curr_adm_pass = st.text_input(
          "Current Password (বর্তমান পাসওয়ার্ড)", type="password"
      )
      new_adm_pass = st.text_input(
          "New Password (নতুন পাসওয়ার্ড)", type="password"
      )
      sub_pass_change = st.form_submit_button(
      ...
      if sub_pass_change:
        ...
        adm_db_p = c.fetchone()
        if adm_db_p and adm_db_p[0] == curr_adm_pass:
          c.execute(
          ...
          st.error("Current password was incorrect.")
