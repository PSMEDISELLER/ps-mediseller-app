...
if current_logged_username != "admin":
    ...
    # Note: If multiple users use the 'delivery' username, blocking 'delivery' blocks everyone. Users need individual usernames.
    c.execute("SELECT is_active FROM users WHERE username=?", (current_logged_username,))
    res_act = c.fetchone()
    if res_act and res_act[0] == 0:
        st.error("🚫 আপনার একাউন্টটি অ্যাডমিন কর্তৃক ব্লক (Block) করা হয়েছে। আপনি এই অ্যাপটি ব্যবহার করতে পারবেন কেজি করা যাবে না।")
        st.stop()
...
with col_u1:
  # Issue 5 FIX: ফ্রন্ট পেজে ইউজারের নাম সুন্দরভাবে হাইলাইট করা হয়েছে (স্টাফ লেখা বাদ দেওয়া হয়েছে)
...
if selected_menu == "📍 Add Location (লোকেশন যোগ)":
  ...
  # Session state init for clearing search fields (Issue 3 Fix)
  ...
  with st.form("order_visit_entry_form", clear_on_submit=True):
    ...
    if submitted_order:
      if not selected_order_party_native.strip():
        ...
        conn.commit()
        # Issue 3 FIX: Clearing search text successfully
        st.session_state["order_party_search_text_input"] = ""
  ...
  with task_tab1:
    ...
    with st.form("easy_assign_form", clear_on_submit=True):
      ...
      # Issue 3 Fix (removing explicitly cached session value for clearing)
  ...
  with task_tab2:
    ...
    st.write("### ⚙️ Settings & Agents Management (কর্মী ইউজার ম্যানেজমেন্ট)")
  ...
  # তিনটি কাজ 3টি আলাদা ট্যাবের মাধ্যমে করা হলো
    tab_db, tab_agents, tab_recycle = st.tabs(["💾 DB Restore", "👥 Agent Management", "♻️ Recycle Bin"])
    
    # কাজ ১: ডিবি ফাইল restore অপশন
    with tab_db:
        st.write("#### 1. DB File Restore (ডিবি ফাইল রিস্টোর)")
        st.warning("⚠️ Warning: Restoring a database will overwrite current data! (পুরনো ডেটা মুছে নতুন ব্যাকআপ ইনস্টল হবে)")
    ...
    # কাজ ২: এজেন্ট নাম ও ফোন নাম্বার এডিট (ড্রপ ডাউন সিস্টেমে)
    with tab_agents:
        ...
        if agents_list:
            # ড্রপডাউনের জন্য একটি ডিকশনারি বানানো হলো যেখানে নাম ও ফোন নাম্বার শো করবে
    ...
    # কাজ ৩: রিসাইকেল বিনের ভেতরে পার্মানেন্ট ডিলেট ও ইনস্টল এর অপশন
    with tab_recycle:
        st.write("#### 3. Recycle Bin (রিসাইকেল বিন)")
        ...
        if deleted_items:
            for item in deleted_items:
                item_id, item_type, item_title, item_data_str, deleted_at = item
                with st.expander(f"📦 {item_type}: {item_title} (Deleted: {deleted_at})"):
                    # JSON Data Show
                    st.code(item_data_str, language='json')
                    
                    col_rb1, col_rb2 = st.columns(2)
                    with col_rb1:
                        # ইনস্টল / রিস্টোর অপশন
                        if st.button("🔄 Install / Restore (পুনরুদ্ধার করুন)", key=f"res_bin_{item_id}"):
                            try:
                                data_dict = json.loads(item_data_str)
                                
                                if item_type == "Location":
                                    c.execute("INSERT INTO locations (party_name, address, party_phone, lat, lon, current_due) VALUES (?, ?, ?, ?, ?, ?)",
                                              (data_dict.get('party_name'), data_dict.get('address'), data_dict.get('party_phone'), data_dict.get('lat'), data_dict.get('lon'), data_dict.get('current_due', 0)))
                                elif item_type == "Order":
                                    c.execute("INSERT INTO orders (party_name, order_details, order_date, status, payment_collected) VALUES (?, ?, ?, ?, ?)",
                                              (data_dict.get('party_name'), data_dict.get('order_details'), data_dict.get('order_date'), data_dict.get('status'), data_dict.get('payment_collected')))
                                    ...
                                    c.execute("INSERT INTO task_assignments (agent_name, party_name, task_type, due_amount, sale_amount, payment_collected_actual, remaining_due, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                              (data_dict.get('agent_name'), data_dict.get('party_name'), data_dict.get('task_type'), data_dict.get('due_amount'), data_dict.get('sale_amount'), data_dict.get('payment_collected_actual'), data_dict.get('remaining_due'), data_dict.get('status'), data_dict.get('created_at')))
                                
                                # রিস্টোর হওয়ার পর রিসাইকেল বিন থেকে ডিলেট করে দেওয়া হবে
                                c.execute("DELETE FROM recycle_bin WHERE id=?", (item_id,))
                                conn.commit()
                                st.success("Item installed/restored successfully! (রিস্টোর করা হয়েছে)")
                                st.rerun()
                    ...
                    with col_rb2:
                        # পার্মানেন্ট ডিলেট অপশন
                        if st.button("🗑️ Permanent Delete (পার্মানেন্ট ডিলেট)", key=f"del_bin_{item_id}", type="primary"):
                            c.execute("DELETE FROM recycle_bin WHERE id=?", (item_id,))
                            conn.commit()
                            st.success("Permanently deleted! (স্থায়ীভাবে ডিলেট করা হয়েছে)")
                            st.rerun()
        else:
