...
div.stExpander details summary, div.stExpander details summary span, div.stExpander details summary p,
[data-testid="stExpander"] summary, [data-testid="stExpander"] summary span, [data-testid="stExpander"] summary p {
...
div[data-testid="stTextInput"] div p, div[data-testid="stTextArea"] div p,
...
# =========================================================
    ...
    with task_tab1:
        ...
        st.write("---")
        st.write("#### 📋 Active Tasks List (চলমান কাজের তালিকা)")
        
        if st.session_state["user_role"] == "admin":
            tasks_query_df = pd.read_sql_query("""
                SELECT t.id, t.agent_name, t.party_name, t.task_type, t.due_amount, t.status, t.created_at, l.address, l.party_phone, l.lat, l.lon 
                FROM task_assignments t 
                LEFT JOIN locations l ON t.party_name = l.party_name 
                WHERE t.status='Pending' 
                ORDER BY t.id DESC
            ...
            tasks_query_df = pd.read_sql_query("""
                ...
                FROM task_assignments t 
                LEFT JOIN locations l ON t.party_name = l.party_name 
                WHERE t.status='Pending' AND t.agent_name=? 
                ORDER BY t.id DESC
            """, conn, params=(st.session_state["username"],))
            
        if not tasks_query_df.empty:
            for idx, row in tasks_query_df.iterrows():
                ag_display = agent_name_map.get(row['agent_name'], row['agent_name'])
                st.markdown(f"""
                <div class="card">
                    <div class="party-title">🏥 {row['party_name']}</div>
                    <div class="card-text">📍 <b>Address:</b> {row['address'] if row['address'] else 'Not specified'}</div>
                    <div class="card-text">📞 <b>Phone:</b> {row['party_phone'] if row['party_phone'] else 'Not specified'}</div>
                    <div class="card-text">📋 <b>Task:</b> {row['task_type']}</div>
                    <div class="card-text">💰 <b>Due Amount:</b> ₹{row['due_amount']}</div>
                    <div class="card-text">👤 <b>Assigned Agent:</b> {ag_display}</div>
                    <div class="card-text">🕒 <b>Time:</b> {row['created_at']}</div>
                </div>
                """, unsafe_allow_html=True)
                ...
                with col_act1:
                    if pd.notna(row['lat']) and pd.notna(row['lon']):
                        maps_url = f"https://www.google.com/maps/dir/?api=1&destination={row['lat']},{row['lon']}"
                        st.markdown(f'<a href="{maps_url}" target="_blank" style="text-decoration:none;"><button style="background: linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%); color:white; border:none; padding:8px 16px; border-radius:8px; cursor:pointer; font-weight:600; width:100%;">🧭 Navigate (ডিরেকশন)</button></a>', unsafe_allow_html=True)
                    else:
                        st.write("⚠️ No Map Coordinates")
                with col_act2:
                    if st.button("✅ Mark Completed (সম্পন্ন করুন)", key=f"complete_task_{row['id']}"):
                        c.execute("UPDATE task_assignments SET status='Completed' WHERE id=?", (row['id'],))
                        conn.commit()
                        if "Due Collection" in row['task_type']:
                        ...
                        conn.commit()
                        st.success("Task completed successfully! (সফল!)")
                        st.rerun()
                st.write("---")
        else:
    ...
    with task_tab2:
        st.write("#### 📊 Agent Date-wise Performance Summary")
        summary_df = pd.read_sql_query("""
            SELECT agent_name, DATE(created_at) as task_date, COUNT(*) as total_tasks, 
        ...
        if not summary_df.empty:
            summary_df['agent_name'] = summary_df['agent_name'].map(lambda x: agent_name_map.get(x, x))
    ...
    with task_tab3:
        ...
        completed_tasks_df = pd.read_sql_query("""
            SELECT t.id, t.agent_name, t.party_name, t.task_type, t.due_amount, t.created_at, l.address 
            FROM task_assignments t 
            LEFT JOIN locations l ON t.party_name = l.party_name 
        ...
        if not completed_tasks_df.empty:
            ...
            for idx, row in completed_tasks_df.iterrows():
                ...
                cols[0].write(f"**{row['party_name']}**")
                cols[1].write(row['task_type'])
                cols[2].write(f"Agent: {ag_display}")
                cols[3].write("✅ Completed")
                st.write("---")
        else:
...
# =========================================================
    ...
    st.write("### 🧭 Delivery Route Map (রুট ম্যাপ)")
    df_locs = pd.read_sql_query("SELECT party_name, address, lat, lon FROM locations WHERE lat IS NOT NULL AND lon IS NOT NULL", conn)
    
    if not df_locs.empty:
        avg_lat = df_locs['lat'].mean()
        avg_lon = df_locs['lon'].mean()
        ...
        st.info("No mapped locations available to generate route map. (ম্যাপযুক্ত লোকেশন নেই।)")
...
# =========================================================
    ...
    with st.form("attendance_form"):
        st.write(f"Date: **{current_date_str}**")
        st.write(f"User: **{current_fullname}**")
        submit_att = st.form_submit_button("✅ Check-In Present (উপস্থিতি दर्ज করুন)", type="primary")
        if submit_att:
            try:
                c.execute(
                    "INSERT INTO attendance (username, date, check_time, status) VALUES (?, ?, ?, ?)",
                    (st.session_state["username"], current_date_str, get_ist_time().strftime("%H:%M:%S"), "Present")
                )
                conn.commit()
                st.success("Attendance recorded successfully! (উপস্থিতি সফলভাবে রেকর্ড হয়েছে!)")
            except sqlite3.IntegrityError:
                st.warning("You have already checked in for today! (আজকে ইতিমধ্যে উপস্থিতি দেওয়া হয়েছে!)")
                
    st.write("---")
    st.write("#### 📋 Attendance History (উপস্থিতির ইতিহাস)")
    att_df = pd.read_sql_query("SELECT * FROM attendance ORDER BY date DESC, id DESC LIMIT 50", conn)
    if not att_df.empty:
...
# =========================================================
    ...
    if st.session_state["user_role"] != "admin":
        st.error("Access Denied! Admin only. (প্রবেশাধিকার সংরক্ষিত!)")
    else:
        st.write("### 📊 Agent Live Tracking & Status (লাইভ ট্র্যাকিং)")
        agents_live_df = pd.read_sql_query("""
        ...
        if not agents_live_df.empty:
            st.dataframe(agents_live_df, use_container_width=True)
            
            avg_lat = agents_live_df['lat'].dropna().mean() if not agents_live_df['lat'].dropna().empty else 22.8620
            avg_lon = agents_live_df['lon'].dropna().mean() if not agents_live_df['lon'].dropna().empty else 87.3320
            
            live_map = folium.Map(location=[avg_lat, avg_lon], zoom_start=13, tiles=None)
            folium.TileLayer(
                tiles="https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}",
                attr="Google Maps Street",
                name="Street View",
            ...
            for idx, row in agents_live_df.iterrows():
                if pd.notna(row['lat']) and pd.notna(row['lon']):
                    name_lbl = row['fullname'] if row['fullname'] else row['username']
                    folium.Marker(
                        [row['lat'], row['lon']],
                        popup=f"<b>Agent: {name_lbl}</b><br>Deliveries: {row['completed_deliveries']}<br>Dues: {row['completed_dues']}<br>Last Active: {row['last_updated']}",
                        tooltip=name_lbl,
                        icon=folium.Icon(color="red", icon="user", prefix="fa")
                    ).add_to(live_map)
                    
            folium.LayerControl().add_to(live_map)
            st_folium(live_map, width="100%", height=450, key="admin_live_tracking_map")
        else:
...
# =========================================================
    ...
    if st.session_state["user_role"] != "admin":
        ...
        st.write("### ⚙️ Settings & User Management (ইউজার ম্যানেজমেন্ট)")
        
        with st.form("create_new_agent_form"):
            st.write("#### 👤 Create New Staff / Agent (নতুন কর্মী তৈরি করুন)")
            ...
            submit_new_user = st.form_submit_button("➕ Create User (ইউজার তৈরি করুন)", type="primary")
            if submit_new_user:
                if new_username.strip() and new_password.strip():
                    try:
                        ...
                        conn.commit()
                        st.success(f"User '{new_username}' created successfully! (সফল!)")
                        st.rerun()
                    except sqlite3.IntegrityError:
                        st.error("Username already exists! (ইউজারনেম ইতিমধ্যে বিদ্যমান!)")
                else:
                    st.error("Username and password are required. (ইউজারনেম ও পাসওয়ার্ড আবশ্যক।)")
                    
        st.write("---")
        st.write("#### 👥 Existing Users List (বিদ্যমান ইউজারদের তালিকা)")
        users_df = pd.read_sql_query("SELECT username, role, fullname, phone, created_at, is_active FROM users", conn)
