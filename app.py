        ...
        function(error) {
            ...
            if (status) {
                ...
                if (error.code === error.PERMISSION_DENIED) {
                    status.innerText = "⚠️ Location permission denied! Please enable it in browser & phone settings.";
                } else if (error.code === error.POSITION_UNAVAILABLE) {
                    status.innerText = "⚠️ GPS signal unavailable. Please turn on phone GPS/Location.";
                } else {
                    status.innerText = "⚠️ Please enable location to continue.";
                }
            }
        },
        {
        ...
        if selected_month.strip():
            ...
            if not all_locs_df.empty:
                ...
                if st.session_state["user_role"] == "admin":
                    ...
                    st.markdown("<p style='color: #60a5fa; font-size: 13px;'><i>Note: Monthly report downloads and management are restricted to admins only.</i></p>", unsafe_allow_html=True)
                ...
                with st.expander(f"👤 Agent: {ag_disp_name} ({len(ag_rows)} Pending Tasks)", expanded=True):
                    for idx, row in ag_rows.iterrows():
                        ...
                        with st.form(key=f"complete_task_form_{row['id']}"):
                            ...
                            with col_f2:
                                payment_input = st.text_input("Payment Collected (কত টাকা পেমেন্ট দিয়েছে)", value=str(row['due_amount']), key=f"pay_amt_{row['id']}")
    ...
    with task_tab3:
        ...
        if not completed_tasks_df.empty:
            if st.session_state["user_role"] == "admin":
                ...
                export_comp_tasks['Agent Name'] = export_comp_tasks.apply(lambda r: r['agent_fullname'] if pd.notna(r['agent_fullname']) and r['agent_fullname'] else r['agent_name'], axis=1)
                export_comp_tasks['Party Name'] = export_comp_tasks['party_name']
                export_comp_tasks['Task Type'] = export_comp_tasks['task_type']
                export_comp_tasks['Sale Amount (₹)'] = export_comp_tasks['sale_amount']
                export_comp_tasks['Collection Amount (₹)'] = export_comp_tasks['payment_collected_actual']
                export_comp_tasks['Due Amount (₹)'] = export_comp_tasks['due_amount']
                export_comp_tasks['Completed Date'] = export_comp_tasks['created_at'].apply(lambda x: format_date_display(x))
                export_comp_tasks['Address'] = export_comp_tasks['address']
                
                export_comp_tasks_final = export_comp_tasks[['Agent Name', 'Party Name', 'Task Type', 'Sale Amount (₹)', 'Collection Amount (₹)', 'Due Amount (₹)', 'Completed Date', 'Address']]
                html_comp_tasks_report = generate_html_report("Completed Tasks & Deliveries Report", export_comp_tasks_final)
                col_ct1, col_ct2 = st.columns(2)
                with col_ct1:
                    st.download_button(
                        label="📥 Download Completed Tasks Report",
                        data=html_comp_tasks_report,
                ...
                with col_ct2:
                    if st.button("🗑️ Clear All Completed Tasks History", type="secondary"):
                        ...
                        conn.commit()
                        st.success("All completed tasks history deleted successfully! (সব মুছে ফেলা হয়েছে!)")
                        st.rerun()
                st.write("---")

            for idx, row in completed_tasks_df.iterrows():
                ag_d_name = row['agent_fullname'] if pd.notna(row['agent_fullname']) and row['agent_fullname'] else row['agent_name']
                st.markdown(f"""
                <div style="background: #1e293b; border: 1px solid rgba(148, 163, 184, 0.35); border-radius: 12px; padding: 16px; margin-bottom: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.3);">
                    <p style="margin: 0 0 6px 0; color: #34d399 !important; font-weight: 700; font-size: 16px;">✅ Completed Task - Party: {row['party_name']}</p>
                    <p style="margin: 0 0 4px 0; color: #cbd5e1 !important; font-size: 13px;">👤 Agent: <b>{ag_d_name}</b> | Task: <code>{row['task_type']}</code></p>
                    <p style="margin: 0 0 4px 0; color: #cbd5e1 !important; font-size: 13px;">💵 Sale: ₹<b>{row['sale_amount']}</b> | Collected: ₹<b>{row['payment_collected_actual']}</b> | Due: ₹<b>{row['due_amount']}</b></p>
                    <p style="margin: 0; color: #94a3b8 !important; font-size: 12px;">📅 Date: {format_date_display(row['created_at'])} {(' | Address: ' + str(row['address'])) if pd.notna(row['address']) else ''}</p>
                </div>
                """, unsafe_allow_html=True)
                if st.session_state["user_role"] == "admin":
...
# =========================================================
    ...
    mapped_locs_df = pd.read_sql_query("SELECT party_name, address, lat, lon, party_phone FROM locations WHERE lat IS NOT NULL AND lon IS NOT NULL ORDER BY party_name ASC", conn)
    if not mapped_locs_df.empty:
        ...
        coords_list = []
        for idx, r in mapped_locs_df.iterrows():
            p_lat, p_lon = r['lat'], r['lon']
            coords_list.append([p_lat, p_lon])
            p_title = r['party_name']
            p_addr = r['address'] if pd.notna(r['address']) else "No address"
            p_ph = r['party_phone'] if pd.notna(r['party_phone']) else "No phone"
            
            popup_html = f"<b>{p_title}</b><br>{p_addr}<br>Ph: {p_ph}<br><a href='https://www.google.com/maps/dir/?api=1&destination={p_lat},{p_lon}' target='_blank'>🧭 Navigate</a>"
            folium.Marker(
                [p_lat, p_lon],
        ...
        if len(coords_list) > 1:
            folium.PolyLine(coords_list, color="#3b82f6", weight=4, opacity=0.8, tooltip="Route Path").add_to(route_map)
        ...
        if gps_lat and gps_lon:
            folium.CircleMarker(
                ...
                fill_color="#34d399",
                fill_opacity=0.9,
                popup="Your Live GPS Location"
            ).add_to(route_map)

        folium.LayerControl().add_to(route_map)
        st_folium(route_map, width="100%", height=500, key="delivery_route_folium_map")
    else:
        st.warning("No mapped parties available for route map. Please add map locations first.")

# =========================================================
# 7. ATTENDANCE
# =========================================================
elif selected_menu == "📅 Attendance (উপস্থিতি)":
    st.write("### 📅 Staff Attendance (উপস্থিতি)")
    today_date_str = get_ist_time().strftime("%Y-%m-%d")
    current_username = st.session_state["username"]

    c.execute("SELECT status, check_time FROM attendance WHERE username=? AND date=?", (current_username, today_date_str))
    att_row = c.fetchone()

    if att_row:
        st.success(f"Attendance recorded for today ({format_date_display(today_date_str)}) at `{att_row[1]}`. Status: **{att_row[0]}**")
    else:
        with st.form("attendance_form"):
            st.write(f"Mark your attendance for today: **{format_date_display(today_date_str)}**")
            submit_att = st.form_submit_button("✅ Give Attendance / Present (উপস্থিত)", type="primary")
            if submit_att:
                check_time_str = get_ist_time().strftime("%H:%M:%S")
                try:
                    c.execute(
                        "INSERT INTO attendance (username, date, check_time, status) VALUES (?, ?, ?, ?)",
                        (current_username, today_date_str, check_time_str, "Present")
                    )
                    conn.commit()
                    st.success("Attendance marked successfully! (উপস্থিতি সংরক্ষিত হয়েছে!)")
                    st.rerun()
                except sqlite3.IntegrityError:
    ...
    if st.session_state["user_role"] == "admin":
        st.write("---")
        st.write("#### 📋 All Attendance Records (সব উপস্থিতি)")
        all_att_df = pd.read_sql_query("""
            SELECT a.date, u.fullname as agent_fullname, a.username, a.check_time, a.status
            FROM attendance a
            LEFT JOIN users u ON a.username = u.username
            ORDER BY a.date DESC, a.check_time DESC
        """, conn)
        if not all_att_df.empty:
            export_att = all_att_df.copy()
            export_att['Date'] = export_att['date'].apply(lambda x: format_date_display(x))
            export_att['Agent Name'] = export_att.apply(lambda r: r['agent_fullname'] if pd.notna(r['agent_fullname']) and r['agent_fullname'] else r['username'], axis=1)
            export_att['Check Time'] = export_att['check_time']
            export_att['Status'] = export_att['status']
            export_att_final = export_att[['Date', 'Agent Name', 'Check Time', 'Status']]

            html_att_report = generate_html_report("Staff Attendance Report", export_att_final)
            col_at1, col_at2 = st.columns(2)
            with col_at1:
                st.download_button(
                    label="📥 Download Attendance Report",
                    data=html_att_report,
                    file_name="mediseller_attendance_report.html",
                    mime="text/html",
                    type="primary"
                )
            with col_at2:
                if st.button("🗑️ Clear All Attendance Records", type="secondary"):
                    c.execute("DELETE FROM attendance")
                    conn.commit()
                    st.success("All attendance records deleted!")
                    st.rerun()
            st.write("---")
            st.dataframe(export_att_final, use_container_width=True)
        else:
...
# =========================================================
    ...
    if agent_locs:
        ...
        for ag in agent_locs:
            uname, fname, alat, alon, l_upd, comp_del = ag
            display_name = fname if fname else uname
            if alat and alon:
                popup_text = f"<b>Agent: {display_name}</b><br>Last Updated: {l_upd}<br>Completed Deliveries: {comp_del}"
                folium.Marker(
                    [alat, alon],
        ...
        st_folium(track_map, width="100%", height=450, key="admin_live_tracking_map")

        st.write("#### 👤 Agent Live Status Table")
        live_df = pd.DataFrame(agent_locs, columns=["Username", "Full Name", "Latitude", "Longitude", "Last Updated", "Completed Deliveries"])
        st.dataframe(live_df, use_container_width=True)
    else:
        st.info("No active agent live location data found.")

# =========================================================
# 9. SETTINGS & AGENTS MANAGEMENT
# =========================================================
elif selected_menu == "⚙️ Settings & Agents (সেটিংসে)" and st.session_state["user_role"] == "admin":
    st.write("### ⚙️ Settings & Agent Management (এজেন্ট ও সেটিংস)")
    set_tab1, set_tab2 = st.tabs(["👥 Manage Agents (কর্মচারী ম্যানেজমেন্ট)", "🗄️ Database Management (ডাটাবেস)"])

    with set_tab1:
        st.write("#### ➕ Add New Agent / User (নতুন কর্মচারী যোগ করুন)")
        with st.form("add_new_user_form", clear_on_submit=True):
            ...
            new_role = st.selectbox("Role (রোল)", ["staff", "admin"])
            
            submit_new_user = st.form_submit_button("💾 Save User (সেভ করুন)", type="primary")
            if submit_new_user:
                if new_uname.strip() and new_pass.strip():
                    try:
                        c.execute(
                        ...
                        conn.commit()
                        st.success(f"User `{new_uname}` added successfully! (সফলভাবে যোগ হয়েছে!)")
                        st.rerun()
                    except sqlite3.IntegrityError:
        ...
        st.write("---")
        st.write("#### 📋 Existing Users List (কর্মচারী তালিকা)")
        users_df = pd.read_sql_query("SELECT username, role, fullname, phone, created_at FROM users", conn)
        for idx, u_row in users_df.iterrows():
            cols = st.columns([2, 2, 2, 2, 1.5])
            cols[0].write(f"**{u_row['username']}**")
            cols[1].write(f"`{u_row['role']}`")
            cols[2].write(u_row['fullname'] if pd.notna(u_row['fullname']) else "-")
            cols[3].write(u_row['phone'] if pd.notna(u_row['phone']) else "-")
            
            if u_row['username'] != "admin":
                if cols[4].button("🗑️ Delete", key=f"del_user_{u_row['username']}"):
                    c.execute("DELETE FROM users WHERE username=?", (u_row['username'],))
                    conn.commit()
                    st.success(f"User `{u_row['username']}` deleted!")
                    st.rerun()
            else:
                cols[4].write("🔒 System")
            st.write("---")
    ...
    with set_tab2:
        st.write("#### 🗄️ Database Backup & Reset Options (ডাটাবেস ব্যাকআপ ও রিসেট)")
        if st.button("📥 Download Database File (.db)", type="primary"):
        ...
        st.write("---")
        if st.button("⚠️ Danger Zone: Reset All Orders / Tasks", type="secondary"):
            c.execute("DELETE FROM orders")
            c.execute("DELETE FROM task_assignments")
            c.execute("DELETE FROM daily_work")
            conn.commit()
            st.success("All operational data reset successfully!")
            st.rerun()
