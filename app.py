...
<div id="loc-overlay" style="display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
background: rgba(15, 23, 42, 0.98); z-index: 999999; justify-content: center; align-items: center; padding: 20px; box-sizing: border-box; font-family: 'Poppins', sans-serif;">
...
div.stExpander details summary, div.stExpander details summary span, div.stExpander details summary p, [data-testid="stExpander"] summary, [data-testid="stExpander"] summary span, [data-testid="stExpander"] summary p {
...
CREATE TABLE IF NOT EXISTS users (
    ...
    allowed_menus TEXT DEFAULT 'Add Location (লোকেশন যোগ), Search & Details (অনুসন্ধান ও বিবরণ), Orders (বাকি অর্ডার), Daily & Monthly Work (দৈনিক ও মাসিক কাজ), Map (রুট ম্যাপ), Attendance (উপস্থিতি), Due & Delivery (বকেয়া ও ডেলিভারি)'
)
""")
...
if "allowed_menus" not in existing_user_cols:
    c.execute("ALTER TABLE users ADD COLUMN allowed_menus TEXT DEFAULT 'Add Location (লোকেশন যোগ), Search & Details (অনুসন্ধান ও বিবরণ), Due & Delivery (বকেয়া ও ডেলিভারি), Pending Orders (বাকি অর্ডার), Daily & Monthly Work (দৈনিক ও মাসিক কাজ), Route Map (রুট ম্যাপ), Attendance (উপস্থিতি)'")
...
def generate_html_report(title, df):
    ...
    @media print {{ .print-btn {{ display: none; }} body {{ background: white; margin: 0; }} table {{ box-shadow: none; }} }}
    </style>
    </head>
    <body>
    <div class="header">
...
if current_logged_username != "admin":
    # Issue 4 FIX: Ensuring block logic applies only to the specific logged-in user.
...
with col_u1:
    # Issue 5 FIX: ফ্রন্ট পেজে ইউজারের নাম সুন্দরভাবে হাইলাইট করা হয়েছে
...
if st.session_state["user_role"] == "admin":
    menu_options = all_basic_menus + ["Live Tracking (লাইভ ট্র্যাকিং)", "Settings & Agents (সেটিংসে)"]
else:
...
if selected_menu == "Add Location (লোকেশন যোগ)":
    ...
    selected_entry_tab = st.radio("Select Entry Mode (মোড সিলেক্ট):", ["With Map Party (ম্যাপ সহ পার্টি)", "Without Map Party (ম্যাপ ছাড়া পার্টি)"], label_visibility="collapsed")
    st.write("")
        ...
        if not doc_df.empty:
            for index, row in doc_df.iterrows():
                ...
                if cols[3].button("Add Map (ম্যাপ যুক্ত)", key=f"map_add_search_{row['id']}"):
                    ...
                    st.rerun()
                if st.session_state["user_role"] == "admin":
                    if cols[4].button("Delete (ডিলিট)", key=f"del_doc_search_{row['id']}"):
    ...
    with st.expander(f"Mapped List ({len(mapped_df)} Records) (ম্যাপযুক্ত তালিকা)", expanded=is_searching):
        if not mapped_df.empty:
            for index, row in mapped_df.iterrows():
                ...
                if st.session_state["user_role"] == "admin":
                    if cols[4].button("Delete (ডিলিট)", key=f"del_loc_search_{row['id']}"):
                        move_to_recycle_bin("Location", row['party_name'], dict(row))
                        c.execute("DELETE FROM locations WHERE id=?", (row['id'],))
                        conn.commit()
                        st.success("Moved to Recycle Bin! (রিসাইকেল বিনে পাঠানো হয়েছে!)")
                ...
                if st.session_state["user_role"] == "admin":
                    ...
                    with col_dc2:
                        if st.button("Clear All Completed Orders History (সব ডিলিট)", type="secondary"):
                            for r in completed_ord_df.iterrows():
                                move_to_recycle_bin("Order", r[1]['party_name'], dict(r[1]))
    ...
    with task_tab1:
        ...
        with st.form("easy_assign_form", clear_on_submit=True):
            ...
            d_amount = st.text_input("Due Amount (ডিউ টাকা)", value=auto_due_val)
            ...
            for ag_username in unique_agents_in_tasks:
                ...
                with st.expander(f"Agent: {ag_disp_name} ({len(ag_rows)} Pending Tasks) - Click to Open", expanded=False):
                    for idx, row in ag_rows.iterrows():
                        ...
                        if st.session_state["user_role"] == "admin":
                            if st.button("Delete Task (টাস্ক ডিলিট)", key=f"del_pend_task_{row['id']}"):
    ...
    with task_tab2:
        ...
        SELECT t.agent_name, u.fullname as agent_fullname, SUBSTR(t.created_at, 1, 10) as task_date, COUNT(t.id) as total_tasks, SUM(CASE WHEN t.status='Completed' THEN 1 ELSE 0 END) as completed_tasks
        FROM task_assignments t
        LEFT JOIN users u ON t.agent_name = u.username
            ...
            for idx, row in agent_sum_df.iterrows():
                ...
                if st.session_state["user_role"] == "admin":
                    resub_toggle = st.checkbox(
                        f"Allow {ag_disp} to Re-submit completed tasks (রি-সাবমিশনের অনুমতি প্রদান)", value=agent_allowed, key=f"resub_{row['agent_name']}_{row['task_date']}"
                    )
...
elif selected_menu == "Settings & Agents (সেটিংসে)":
    if st.session_state["user_role"] != "admin":
        st.error("Admin access required! (অ্যাডমিন পারমিশন প্রয়োজন)")
        st.stop()
        
    st.write("### Settings & Agents Management (কর্মী ইউজার ম্যানেজমেন্ট)")
    ...
    tab_db, tab_agents, set_tab1, set_tab_perm, set_tab3, set_tab4, set_tab5, set_tab6 = st.tabs([
        "DB Restore",
        "Agent Management",
        "Add Agents & Links",
        "Menu Permissions",
    ...
    with tab_db:
        st.write("#### 1. DB File Restore (ডিবি ফাইল রিস্টোর)")
        st.warning("Warning: Restoring a database will overwrite current data! (পুরনো ডেটা মুছে নতুন ব্যাকআপ ইনস্টল হবে)")
        uploaded_db = st.file_uploader("Upload database file (.db) (ডিবি ফাইল আপলোড করুন)", type=["db"])
        if uploaded_db is not None:
            if st.button("Restore Database (রিস্টোর করুন)", type="primary"):
                try:
                    with open(DB_FILE, "wb") as f:
                        f.write(uploaded_db.getbuffer())
                    st.success("Database restored successfully! (সফলভাবে রিস্টোর হয়েছে)")
                    st.rerun()
                except Exception as e:
                    st.error(f"Error restoring DB: {e}")
                    
    with tab_agents:
        st.write("#### 2. Agent Details Edit (এজেন্ট নাম ও ফোন নাম্বার আপডেট)")
        c.execute("SELECT username, fullname, phone FROM users WHERE role='staff'")
        agents_list = c.fetchall()
        if agents_list:
            agent_map = {f"{a[1] or a[0]} - {a[2] or 'No Phone'} ({a[0]})": a[0] for a in agents_list}
            selected_agent_label = st.selectbox("Select Agent (এজেন্ট সিলেক্ট করুন)", list(agent_map.keys()))
            selected_username = agent_map[selected_agent_label]
            
            c.execute("SELECT fullname, phone FROM users WHERE username = ?", (selected_username,))
            curr_details = c.fetchone()
            
            with st.form("agent_edit_form"):
                new_fullname = st.text_input("Agent Name (এজেন্ট নাম এডিট)", value=curr_details[0] if curr_details[0] else "")
                new_phone = st.text_input("Phone Number (ফোন নাম্বার এডিট)", value=curr_details[1] if curr_details[1] else "")
                if st.form_submit_button("Update Agent (আপডেট করুন)", type="primary"):
                    c.execute("UPDATE users SET fullname=?, phone=? WHERE username=?", (new_fullname, new_phone, selected_username))
                    conn.commit()
                    st.success("Agent details updated! (এজেন্ট ডিটেলস আপডেট হয়েছে)")
                    st.rerun()
        else:
            st.info("No delivery agents found. (কোনো এজেন্ট পাওয়া যায়নি)")
