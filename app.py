...
# Table Updates (Schema Alterations)
...
input, textarea, select, [data-baseweb="input"] input, div[data-baseweb="input"] {
...
# ==========================================
# 5. FAST USER SESSION & AUTHENTICATION
# ==========================================
# URL প্যারামিটার থেকে লগইন আইডি সংগ্রহ
url_user = st.query_params.get("login", None)
if isinstance(url_user, list):
    url_user = url_user[0] if url_user else None

# Session State হ্যান্ডলিং (হ্যাংমুক্ত ও ফাস্ট)
if "username" not in st.session_state:
    st.session_state["username"] = url_user if url_user else "staff"

if url_user and st.session_state["username"] != url_user:
    st.session_state["username"] = url_user

current_user = st.session_state["username"]

# ডেটাবেস থেকে ইউজারের ডাটা চেক
c.execute("SELECT fullname, role, is_active FROM users WHERE username=?", (current_user,))
user_row = c.fetchone()

if user_row:
    f_name, r_role, is_active = user_row
    if is_active == 0:
        st.error("আপনার একাউন্টটি ব্লক করা হয়েছে।")
        st.session_state["username"] = "staff"
        st.session_state["user_role"] = "staff"
        st.stop()
    else:
        st.session_state["user_role"] = r_role
        st.session_state["display_name"] = f_name if f_name else current_user
        st.query_params["login"] = current_user
else:
    st.session_state["username"] = "staff"
    st.session_state["user_role"] = "staff"
    st.session_state["display_name"] = "Staff Agent"
