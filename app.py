...
if current_logged_username != "admin":
    ...
    if res_act and res_act[0] == 0:
        st.error("🚫 আপনার একাউন্টটি অ্যাডমিন কর্তৃক ব্লক (Block) করা হয়েছে। আপনি এই অ্যাপটি ব্যবহার করতে পারবেনবিধা।")
        st.stop()
...
if selected_menu == "📍 Add Location (লোকেশন যোগ)":
  ...
  with task_tab1:
    ...
    if not pending_tasks_df.empty:
      ...
      st.info("No active pending tasks. (কোনো কাজ বাকি নেই।)")

  with task_tab2:
    st.write("#### 📊 Agent Date-wise Summary (কর্মচারী ও তারিখ অনুযায়ী সামারি)")
    summary_df = pd.read_sql_query("""
        SELECT u.fullname as 'Agent Name', DATE(t.created_at) as 'Date', t.status as 'Status', COUNT(t.id) as 'Total Tasks', 
        SUM(CAST(t.due_amount AS REAL)) as 'Total Due Assigned', SUM(CAST(t.payment_collected_actual AS REAL)) as 'Total Collected'
        FROM task_assignments t
        LEFT JOIN users u ON t.agent_name = u.username
        GROUP BY u.fullname, DATE(t.created_at), t.status 
        ORDER BY 'Date' DESC
    """, conn)
  ...
  with task_tab3:
    ...
    comp_tasks_df = pd.read_sql_query("""
        SELECT t.id, u.fullname as 'Agent Name', t.party_name as 'Party Name', t.task_type as 'Task Type', 
        t.due_amount as 'Assigned Due', t.sale_amount as 'Sale', t.payment_collected_actual as 'Collected', 
        t.remaining_due as 'Remaining Due', t.created_at as 'Date'
        FROM task_assignments t
        LEFT JOIN users u ON t.agent_name = u.username
    ...
    if not comp_tasks_df.empty:
      st.dataframe(comp_tasks_df, use_container_width=True)
    else:
      st.info("No completed tasks found.")

  with task_tab4:
    st.write("#### 💰 Master Due List (বকেয়া তালিকা)")
    due_df = pd.read_sql_query("SELECT party_name as 'Party Name', current_due as 'Current Due', party_phone as 'Phone', address as 'Address' FROM locations WHERE current_due > 0 ORDER BY party_name ASC", conn)
    if not due_df.empty:
      st.dataframe(due_df, use_container_width=True)
    else:
      st.success("No dues pending! (কোনো বকেয়া নেই!)")

elif selected_menu == "🗺️ Route Map (রুট ম্যাপ)":
  st.write("### 🗺️ Route Map (রুট ম্যাপ)")
  map_df = pd.read_sql_query("SELECT party_name, lat, lon, address FROM locations WHERE lat IS NOT NULL AND lon IS NOT NULL", conn)
  if not map_df.empty:
      avg_lat = map_df["lat"].mean()
      avg_lon = map_df["lon"].mean()
      r_map = folium.Map(location=[avg_lat, avg_lon], zoom_start=13)
      for _, r in map_df.iterrows():
          folium.Marker([r["lat"], r["lon"]], popup=f"<b>{r['party_name']}</b><br>{r['address']}").add_to(r_map)
      st_folium(r_map, width="100%", height=500)
  else:
      st.info("No mapped locations available to show route. (রুট দেখানোর জন্য ম্যাপ করা লোকেশন নেই।)")

elif selected_menu == "📅 Attendance (উপস্থিতি)":
  st.write("### 📅 Attendance (উপস্থিতি)")
  cur_date = get_ist_time().strftime("%Y-%m-%d")
  c.execute("SELECT status, check_time FROM attendance WHERE username=? AND date=?", (st.session_state["username"], cur_date))
  att = c.fetchone()
  
  if not att:
      st.info("You haven't checked in today. (আপনি আজকে হাজিরা দেননি।)")
      if st.button("✅ Check In (উপস্থিতি দিন)", type="primary"):
          c.execute("INSERT INTO attendance (username, date, check_time, status) VALUES (?, ?, ?, 'Present')",
                    (st.session_state["username"], cur_date, get_ist_time().strftime("%H:%M:%S")))
          conn.commit()
          st.success("Checked in successfully! (হাজিরা সম্পূর্ণ!)")
          st.rerun()
  else:
      st.success(f"✅ You checked in today at {att[1]}! Status: {att[0]} (হাজিরা সম্পন্ন হয়েছে!)")

  if st.session_state["user_role"] == "admin":
      st.write("---")
      st.write("#### 📋 Staff Attendance Report (কর্মচারীদের হাজিরার রিপোর্ট)")
      att_df = pd.read_sql_query("""
          SELECT u.fullname as 'Agent Name', a.date as 'Date', a.check_time as 'Time', a.status as 'Status' 
          FROM attendance a 
  ...
  st.write("### 📊 Agent Live Tracking (লাইভ ট্র্যাকিং)")
  c.execute("SELECT a.username, a.lat, a.lon, a.last_updated, u.fullname FROM agent_live_locations a JOIN users u ON a.username = u.username")
  agents_loc = c.fetchall()
  
  if agents_loc:
      avg_lat = sum([a[1] for a in agents_loc]) / len(agents_loc)
      avg_lon = sum([a[2] for a in agents_loc]) / len(agents_loc)
      m = folium.Map(location=[avg_lat, avg_lon], zoom_start=13)
      for al in agents_loc:
          popup_text = f"<b>{al[4]}</b> ({al[0]})<br>Last seen: {al[3]}"
          folium.Marker([al[1], al[2]], popup=popup_text, icon=folium.Icon(color='blue', icon='user')).add_to(m)
      st_folium(m, width="100%", height=500)
  else:
      st.info("No active agent locations found. (কোনো কর্মচারীর লোকেশন পাওয়া যায়নি।)")

elif selected_menu == "⚙️ Settings & Agents (সেটিংসে)":
  st.write("### ⚙️ Settings & Agent Management (সেটিংসে ও এজেন্ট পরিচালনা)")
  ...
  with set_tab1:
     st.write("#### 👤 Manage Agents & Login Links")
     
     c.execute("SELECT username, fullname, phone FROM users WHERE role='staff'")
     agents = c.fetchall()
     agent_options = ["➕ Create New Agent (নতুন তৈরি করুন)"] + [f"{a[0]} (Name: {a[1]})" for a in agents]
     selected_agent_opt = st.selectbox("Select Agent to Edit or Add New (এজেন্ট সিলেক্ট করুন):", agent_options)

     if selected_agent_opt == "➕ Create New Agent (নতুন তৈরি করুন)":
         with st.form("add_agent_form", clear_on_submit=True):
             st.write("**Create New Agent (নতুন এজেন্ট তৈরি করুন):**")
             new_uname = st.text_input("Username (Login ID) - *without space*")
             new_pass = st.text_input("Password (পাসওয়ার্ড)", type="password")
             new_fname = st.text_input("Full Name (ফ্রন্ট পেজে এই নামটি দেখাবে)")
             new_phone = st.text_input("Phone Number (ফোন নম্বর)")
             if st.form_submit_button("➕ Add Agent (যুক্ত করুন)", type="primary"):
                 if new_uname.strip() and new_pass.strip() and new_fname.strip():
                     try:
                         c.execute("INSERT INTO users (username, password, role, fullname, phone, created_at, is_active, allow_resubmit) VALUES (?, ?, ?, ?, ?, ?, 1, 0)",
                                   (new_uname.strip(), new_pass.strip(), "staff", new_fname.strip(), new_phone.strip(), get_ist_time().strftime("%Y-%m-%d %H:%M:%S")))
                         conn.commit()
                         st.success(f"Agent '{new_fname}' created successfully! (সফলভাবে তৈরি হয়েছে!)")
                         st.rerun()
                     except sqlite3.IntegrityError:
         ...
         sel_u = selected_agent_opt.split(" ")[0]
         c.execute("SELECT username, password, fullname, phone FROM users WHERE username=?", (sel_u,))
         u_data = c.fetchone()
         
         with st.form("edit_agent_form"):
             st.write(f"**Edit details for '{sel_u}':**")
             e_fname = st.text_input("Full Name (ফ্রন্ট পেজে এই নামটি দেখাবে)", value=u_data[2] if u_data[2] else "")
             e_phone = st.text_input("Phone Number (ফোন নম্বর)", value=u_data[3] if u_data[3] else "")
             e_pass = st.text_input("New Password (খালি রাখলে আগেরটাই থাকবে)", type="password")
             
             if st.form_submit_button("💾 Update Details (আপডেট করুন)", type="primary"):
                 final_pass = e_pass.strip() if e_pass.strip() else u_data[1]
                 c.execute("UPDATE users SET fullname=?, phone=?, password=? WHERE username=?", (e_fname.strip(), e_phone.strip(), final_pass, sel_u))
                 conn.commit()
                 st.success("Agent details updated successfully! (আপডেট হয়েছে!)")
                 st.rerun()
                 
         st.write("🔗 **Login Link for this Agent:**")
         base_url = streamlit_js_eval(js_expressions="window.location.href.split('?')[0]", key="get_base_url_for_link")
         if base_url:
             auto_login_link = f"{base_url}?login={sel_u}"
             st.code(auto_login_link)
             st.info("👆 Copy and send this link to the agent to auto-login. (লিঙ্কটি কপি করে পাঠিয়ে দিন)")

  with set_tab2:
     st.write("#### 🔐 Menu Permissions")
     c.execute("SELECT username, fullname FROM users WHERE role='staff' AND is_active=1")
     staff_users = c.fetchall()
     if staff_users:
         staff_options = {u[0]: f"{u[0]} (Name: {u[1]})" for u in staff_users}
         selected_staff_uname = st.selectbox("Select Agent for Permission (এজেন্ট সিলেক্ট করুন):", list(staff_options.keys()), format_func=lambda x: staff_options[x])
         
         c.execute("SELECT allowed_menus FROM users WHERE username=?", (selected_staff_uname,))
         curr_menus = c.fetchone()[0]
         curr_menu_list = curr_menus.split(",") if curr_menus else []

         with st.form("menu_perm_form"):
             selected_menus = st.multiselect("Allowed Menus (অনুমোদিত মেনুসমূহ):", all_basic_menus, default=[m for m in curr_menu_list if m in all_basic_menus])
             if st.form_submit_button("💾 Update Permissions (আপডেট করুন)", type="primary"):
                 new_menus_str = ",".join(selected_menus)
                 c.execute("UPDATE users SET allowed_menus=? WHERE username=?", (new_menus_str, selected_staff_uname))
                 conn.commit()
                 st.success(f"Permissions updated for {selected_staff_uname}! (আপডেট হয়েছে!)")
                 st.rerun()
     else:
         st.info("No active agents found.")

  with set_tab3:
     st.write("#### 🚫 Unknown / Blocked Agents (বাদ দেওয়া এজেন্টসমূহ)")
     c.execute("SELECT username, fullname, phone FROM users WHERE is_active=0 AND role='staff'")
     blocked_users = c.fetchall()
     if blocked_users:
         for bu in blocked_users:
             cols = st.columns([3, 2, 2])
             cols[0].write(f"**{bu[0]}** (Name: {bu[1]})")
             if cols[1].button("✅ Restore / Unblock (পুনরায় যুক্ত করুন)", key=f"unblock_{bu[0]}"):
                 c.execute("UPDATE users SET is_active=1 WHERE username=?", (bu[0],))
                 conn.commit()
                 st.success(f"Agent {bu[0]} restored!")
                 st.rerun()
             if cols[2].button("🗑️ Delete Permanently (পার্মানেন্ট ডিলিট)", key=f"delperm_{bu[0]}"):
                 c.execute("DELETE FROM users WHERE username=?", (bu[0],))
                 conn.commit()
                 st.success(f"Agent {bu[0]} deleted permanently!")
                 st.rerun()
             st.write("---")
     else:
         st.info("No blocked or unknown agents. (কোনো ব্লক করা এজেন্ট নেই।)")

  with set_tab4:
     st.write("#### 💾 Backup & Restore Database")
     st.write("**Backup:** Download your current database for safety.")
     with open(DB_FILE, "rb") as f:
         st.download_button("📥 Download Backup (.db file)", data=f, file_name="mediseller_backup.db", type="primary")

     st.write("---")
     st.write("**Restore:** Upload a previous backup file to restore data.")
     uploaded_db = st.file_uploader("Upload .db Backup File (ব্যাকআপ ফাইল আপলোড করুন)", type=['db'])
     if uploaded_db is not None:
         if st.button("⚠️ Restore Now (রিস্টোর করুন)"):
             with open(DB_FILE, "wb") as f:
                 f.write(uploaded_db.getbuffer())
             st.success("Database restored successfully! Please reload/refresh the app page. (সফলভাবে রিস্টোর হয়েছে! পেজটি রিফ্রেশ করুন।)")

  with set_tab5:
     st.write("#### 🗑️ Recycle Bin (ডিলিট হওয়া তথ্য)")
     c.execute("SELECT id, item_type, item_title, item_data, deleted_at FROM recycle_bin ORDER BY deleted_at DESC")
     rb_items = c.fetchall()
     
     if rb_items:
         if st.button("❌ Empty Recycle Bin (সব পার্মানেন্ট ডিলিট করুন)"):
         ...
         for item in rb_items:
             ...
             cols[0].write(f"**[{item[1]}]**")
             cols[1].write(f"{item[2]} <br><small>{item[4]}</small>", unsafe_allow_html=True)
             
             if cols[2].button("🔄 Restore (রিস্টোর)", key=f"res_{item[0]}"):
                 data = json.loads(item[3])
                 if item[1] == "Location":
                     c.execute("INSERT OR IGNORE INTO locations (id, party_name, address, party_phone, lat, lon, current_due) VALUES (?, ?, ?, ?, ?, ?, ?)",
                               (data.get('id'), data.get('party_name'), data.get('address'), data.get('party_phone'), data.get('lat'), data.get('lon'), data.get('current_due', 0)))
                 elif item[1] == "Order":
                     c.execute("INSERT OR IGNORE INTO orders (id, party_name, order_details, order_date, status, payment_collected) VALUES (?, ?, ?, ?, ?, ?)",
                               (data.get('id'), data.get('party_name'), data.get('order_details'), data.get('order_date'), data.get('status'), data.get('payment_collected')))
                 elif item[1] == "Daily Work":
                     c.execute("INSERT OR IGNORE INTO daily_work (id, party_name, activity_type, work_date) VALUES (?, ?, ?, ?)",
                               (data.get('id'), data.get('party_name'), data.get('activity_type'), data.get('work_date')))
                 elif item[1] == "Task":
                     c.execute("INSERT OR IGNORE INTO task_assignments (id, agent_name, party_name, task_type, due_amount, sale_amount, payment_collected_actual, remaining_due, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                               (data.get('id'), data.get('agent_name'), data.get('party_name'), data.get('task_type'), data.get('due_amount'), data.get('sale_amount'), data.get('payment_collected_actual'), data.get('remaining_due'), data.get('status'), data.get('created_at')))
                 
                 c.execute("DELETE FROM recycle_bin WHERE id=?", (item[0],))
                 conn.commit()
                 st.success("Item restored! (রিস্টোর হয়েছে!)")
                 st.rerun()
                 
             if cols[3].button("❌ Delete (পার্মানেন্ট)", key=f"perm_{item[0]}"):
                 c.execute("DELETE FROM recycle_bin WHERE id=?", (item[0],))
                 conn.commit()
