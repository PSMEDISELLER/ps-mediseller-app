...
def generate_html_report(title, dataframe):
    safe_now = datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)
...
def init_supabase() -> Client:
    if not SUPABASE_URL or not SUPABASE_URL.startswith("https://"):
        st.error("Invalid or missing SUPABASE_URL in Streamlit Secrets!")
        st.stop()
    if not SUPABASE_KEY:
        st.error("Missing SUPABASE_KEY in Streamlit Secrets!")
        st.stop()
        ...
        function (error) {
            ...
            if (status) {
                ...
                if (error.code === error.PERMISSION_DENIED) {
                    status.innerText = "Location permission denied! Please enable it in browser & phone settings. (লোকেশন পারমিশন দিন)";
                } else if (error.code === error.POSITION_UNAVAILABLE) {
                    status.innerText = "GPS signal unavailable. Please turn on phone GPS/Location. (জিপিএস অন করুন)";
                } else {
                    status.innerText = "Please enable location to continue. (লোকেশন অন করুন)";
                }
            }
        },
        {
...
div[data-testid="stTextInput"] small, div[data-testid="stTextArea"] small, div[data-testid="stTextInput"] div p, div[data-testid="stTextArea"] div p, .stTextInput small, .stTextArea small {
...
def get_gps_component():
    ...
    with open(os.path.join(tmpdir, "index.html"), "w", encoding="utf-8") as f:
        ...
        #getLocBtn {
            background-color: #1a73e8; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer; font-size: 14px; font-weight: bold; width: 100%; text-align: center;
        }
...
# Session state-এ ল্যাটিটিউড এবং লঙ্গিটিউড সেট করা
if "selected_lat" not in st.session_state:
...
if selected_menu == "Add Location (লোকেশন যোগ)":
    ...
    if st.session_state.get("user_role") == "admin":
        with st.expander("🔑 Admin: Manage Routes (রুট ম্যানেজ করুন)"):
            ...
            with col_ar1:
                new_route_admin = st.text_input("Add Route", key="admin_new_route", label_visibility="collapsed", placeholder="নতুন রুট টাইপ করে সেভ করুন...")
            with col_ar2:
            ...
            with col_er1:
                route_to_edit = st.selectbox("Select Route to Edit", [""] + existing_routes, key="admin_edit_route_sel", label_visibility="collapsed")
            with col_er2:
                updated_route_name = st.text_input("New Route Name", key="admin_updated_route_name", label_visibility="collapsed", placeholder="নতুন নাম লিখুন...")
            with col_er3:
                    ...
                    if route_to_edit and updated_route_name.strip():
                        try:
                            ...
                            if chk.data:
                                ...
                                st.success(f"Route updated to '{updated_route_name.strip()}' successfully!")
                                st.rerun()
                        except Exception as e:
                            st.error(f"Error: {e}")
                    else:
                        st.error("Select route & enter new name!")

            st.write("---")
            st.write("**Delete Route (রুট ডিলিট করুন)**")
            col_dr1, col_dr2 = st.columns([3, 1])
            with col_dr1:
                route_to_delete = st.selectbox("Select Route to Delete", [""] + existing_routes, key="admin_del_route", label_visibility="collapsed")
            with col_dr2:
                if st.button("Delete Route", key="admin_delete_route_btn"):
                    if route_to_delete:
                        try:
                            supabase.table("routes").delete().eq("route_name", route_to_delete).execute()
                            st.success(f"Route deleted! ({route_to_delete} ডিলিট হয়েছে)")
                            st.rerun()
                        except Exception as e:
                            st.error(f"Error: {e}")
                    else:
                        st.error("Select a route!")
        ...
        if gps_data and isinstance(gps_data, dict) and "lat" in gps_data:
            if st.session_state.get("last_processed_gps") != gps_data:
                ...
                if round(st.session_state.get("selected_lat", 0), 6) != round(new_lat, 6) or round(st.session_state.get("selected_lon", 0), 6) != round(new_lon, 6):
                    ...
                    st.toast("High-accuracy GPS location taken!", icon="📍")
                    st.rerun()
...
# ==============================================================================
        ...
        with col_b1:
            if st.button("💾 Save Location (সেভ করুন)", type="primary", key="save_party_map_ok"):
                try:
                    ...
                    st.error(f"Error updating map location: {e}")
    ...
    if st.session_state.get("editing_party_id") and st.session_state.get("user_role") == "admin":
...
if st.session_state.get("user_role") == "admin" and not df.empty:
    if "party_name" in df.columns and "address" in df.columns and "party_phone" in df.columns:
...
with st.expander(f"📍 Non-Map List ({len(doc_df)} Entries) (ম্যাপবিহীন তালিকা)", expanded=is_searching):
    if not doc_df.empty:
        for index, row in doc_df.iterrows():
            ...
            cols[1].markdown(f"📞 {row['party_phone']}" if row.get('party_phone') else "No number")
            cols[2].markdown(f"🏠 {row['address']}" if row.get('address') else "No address")
...
st.write("---")
st.markdown("### Route-wise Party (রুট অনুযায়ী পার্টি)")
    ...
    with st.expander(f"📍 {selected_route} Non-Map List ({len(route_doc_df)} Entries) (ম্যাপবিহীন তালিকা)", expanded=True):
        if not route_doc_df.empty:
            for index, row in route_doc_df.iterrows():
                ...
                if st.session_state.get("user_role") == "admin":
                    if cols[3].button("Edit", key=f"edit_doc_route_{row['id']}"):
                        st.session_state["editing_party_id"] = row['id']
                        st.rerun()
    ...
    with st.expander(f"🗺️ {selected_route} Mapped List ({len(route_mapped_df)} Records) (ম্যাপযুক্ত তালিকা)", expanded=True):
        if not route_mapped_df.empty:
            for index, row in route_mapped_df.iterrows():
                ...
                if st.session_state.get("user_role") == "admin":
                    cols[3].markdown(f'<a href="{maps_url}" target="_blank" style="text-decoration: none;"><button style="background: linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%); color: white; border: none; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-weight: 600;">Direction</button></a>', unsafe_allow_html=True)
                    if cols[4].button("Edit", key=f"edit_loc_route_{row['id']}"):
                        st.session_state["editing_party_id"] = row['id']
                        st.rerun()
        ...
        with ord_tab2:
            ...
            if not completed_ord_df.empty:
                    ...
                    with col_dc2:
                        if st.button("🗑️ Clear All Completed Orders History (সব ডিলিট)", type="secondary"):
                            try:
                                ...
                                st.success("All completed orders history moved to Recycle Bin!")
                                st.rerun()
                            except Exception as e:
                                st.error(f"Error: {e}")
                ...
                for idx, row in completed_ord_df.iterrows():
                    cols = st.columns([2, 4, 2, 1.5] if st.session_state.get("user_role") == "admin" else [2, 4, 2])
                    cols[0].write(f"**{row['party_name']}**")
                    cols[1].write(row['order_details'])
                    ...
                    if st.button("🗑️ Clear All Daily Work Records (সব কাজ মুছুন)", type="secondary"):
                        try:
                            ...
                            if all_dw_res.data:
                                ...
                                st.success("All daily work records moved to Recycle Bin!")
                                st.rerun()
                        except Exception as e:
                            st.error(f"Error: {e}")
        ...
        if not work_df.empty:
            ...
            for d_str in unique_dates:
                ...
                with st.expander(f"📅 Date: {formatted_d} (Total: {count_parties}) Click to Open", expanded=False):
                    ...
                    st.write("---")
                    for idx, w_row in date_records.iterrows():
                        cols = st.columns([3, 2, 1.5] if st.session_state.get("user_role") == "admin" else [3, 2])
        ...
        if selected_month.strip():
            ...
            if not all_locs_df.empty:
                ...
                for idx, loc_row in all_locs_df.iterrows():
                    ...
                    v_count = sum(1 for d in month_dw_data if d.get("party_name") == p_name and "Visit" in str(d.get("activity_type")))
                    o_count = sum(1 for d in month_dw_data if d.get("party_name") == p_name and "Order" in str(d.get("activity_type")))
...
with task_tab1:
    ...
    if st.session_state.get("user_role") == "admin":
        try:
            ...
            st.error(f"রিপোর্ট তৈরিতে সাময়িক সমস্যা হয়েছে: {e}")
    ...
    with st.container(border=True):
        ...
        with st.form("easy_assign_form", clear_on_submit=True):
            ...
            try:
                u_res = supabase.table("users").select("username, fullname").execute()
                agent_data = u_res.data if (u_res and u_res.data) else []
                all_agents = [u["username"] for u in agent_data if u.get("username")]
                agent_name_map = {u["username"]: u.get("fullname", u["username"]) for u in agent_data if u.get("username")}
            except Exception:
                all_agents = []
                agent_name_map = {}
            ...
            if submit_easy_task:
                if not sel_pt or not str(sel_pt).strip():
                    st.error("অনুগ্রহ করে একটি নির্দিষ্ট পার্টি সিলেক্ট করুন!")
                else:
                    ...
                    try:
                        ...
                        st.error(f"ডাটাবেসে টাস্ক সংরক্ষণে সমস্যা: {e}")
    ...
    with tab_delivery:
        if del_df.empty:
            ...
            for ag_username in del_df["agent_name"].unique():
                ...
                ag_disp_name = safe_str(ag_rows.iloc[0].get("agent_fullname")) or ag_username
                            ...
                            with st.form(key=f"complete_task_form_del_{task_id}", clear_on_submit=True):
                                ...
                                if submit_complete:
                                    if not sale_input.strip():
                                        st.error("নতুন বিক্রয় (Sale Amount) ইনপুট দেওয়া বাধ্যতামূলক!")
                                    else:
                                        final_sale = safe_float(sale_input)
                                        p_amt = safe_float(payment_input)
                                        if final_sale <= 0:
                                            st.error("বিক্রয়ের মান সঠিক সংখ্যায় (০-এর চেয়ে বড়) হতে হবে!")
                                        else:
                                            ...
                                            try:
                                                ...
                                                st.error(f"আপডেট করতে সমস্যা হয়েছে: {e}")
                            ...
                            if st.session_state.get("user_role") == "admin":
                                if st.button("🗑️ Delete Task", key=f"del_task_del_btn_{task_id}"):
                                    try:
                                        ...
                                        st.toast("টাস্ক মুছে রিসাইকেল বিনে পাঠানো হয়েছে!", icon="🗑️")
                                        st.rerun()
                                    except Exception as e:
                                        st.error(f"টাস্ক মুছতে ব্যর্থ: {e}")
...
with task_tab2:
    ...
    try:
        ...
        t_list = all_tasks_res.data if (all_tasks_res and all_tasks_res.data) else []
    except Exception:
        t_list = []
    ...
    if t_list:
        try:
            ...
            users_map = {u["username"].strip().lower(): u for u in (users_res.data or []) if u.get("username")}
        except Exception:
            users_map = {}
    ...
    if not agent_sum_df.empty:
        ...
        for idx, row in agent_sum_df.iterrows():
            ...
            <div style="background: #1e293b; border: 1px solid rgba(148, 163, 184, 0.35); border-radius: 12px; padding: 16px; margin-bottom: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.3);">
                <p style="margin: 0 0 6px 0; color: #38bdf8 !important; font-size: 16px; font-weight: 600;">Agent: {ag_disp}</p>
                ...
                with st.expander(f"🔄 Re-submission Option (ভুলবশত কমপ্লিট হওয়া কাজ পুনরায় একটিভ করুন {len(comp_tasks_df)})", expanded=False):
                    ...
                    if not can_do_resubmit:
                        ...
                        for ct_idx, ct_row in comp_tasks_df.iterrows():
                            st.markdown(f"**Party:** {ct_row['party_name']} | **Type:** {ct_row['task_type']} | **Collected:** `৳ {ct_row.get('payment_collected_actual', 0)}`")
                            if st.button("Move to Active Tasks (পুনরায় একটিভ করুন)", key=f"btn_resub_ok_{ct_row['id']}_{idx}_{ct_idx}"):
                                try:
...
with task_tab3:
    ...
    if is_admin:
        with st.expander("🔑 Manage Agent PDF Download Permissions (এজেন্টদের ডাউনলোডের অনুমতি দিন)"):
            try:
                ...
                all_agents_db = [r["username"] for r in u_data if r.get("username")]
                user_display_map = {r["username"]: (r.get("fullname") if r.get("fullname") else r["username"]) for r in u_data if r.get("username")}
            except Exception:
                all_agents_db = []
                user_display_map = {}
            ...
            try:
                ...
                allowed_list = [r["username"] for r in (perm_res.data or []) if r.get("username")]
            except Exception:
                allowed_list = []
            ...
            if st.button("Save Permissions (পারমিশন সেভ করুন)", type="primary"):
                try:
                    ...
                    st.error(f"পারমিশন সেভ করতে সমস্যা: {e}")

    # 2. CHECK PERMISSIONS FOR NON-ADMIN
    if is_admin:
        can_download = True
    else:
        try:
    ...
    if ct_data:
        try:
            users_res = supabase.table("users").select("username, fullname").execute()
            users_map = {u["username"].lower(): u.get("fullname") for u in (users_res.data or []) if u.get("username")}
            
            locs_res = supabase.table("locations").select("party_name, address, current_due").execute()
            locs_map = {l["party_name"]: l for l in (locs_res.data or []) if l.get("party_name")}
        except Exception:
            users_map = {}
            locs_map = {}
        ...
        completed_tasks_df = pd.DataFrame(ct_data)
    else:
    ...
    if not completed_tasks_df.empty:
        ...
        with col_f2:
            ...
            selected_agent = st.selectbox("Select Agent (এজেন্ট সিলেক্ট করুন)", options=agent_list)
            ...
            if can_download:
                ...
                with col_tc1:
                    ...
                    if pisa:
                        ...
                        st.warning("xhtml2pdf প্যাকেজ ইনস্টল নেই।")
                    ...
                    if is_admin:
                        if st.button("🗑️ Clear Filtered Tasks History", type="secondary"):
                            try:
                                ...
                                st.error(f"মুছে ফেলতে সমস্যা: {e}")
            else:
                st.info("ℹ️ আপনার PDF ডাউনলোড করার অনুমতি নেই। প্রয়োজনে অ্যাডমিনের সাথে যোগাযোগ করুন।")
                    ...
                    if unique_months:
                        ...
                        if st.button(f"Delete All Data for {selected_month_to_delete}", type="primary"):
                            try:
                                ...
                                st.error(f"বাল্ক ডিলিট করতে সমস্যা: {e}")
                    else:
...
# ==============================================================================
# MENU / ROUTE ROUTING LOGIC (PAGE 5)
# ==============================================================================
...
if selected_menu == "Master Due List (পার্টি ডিউ)":
    ...
    try:
        if party_search_query.strip():
            ...
            due_res = supabase.table("locations").select("party_name, current_due").ilike("party_name", q_term).order("party_name", desc=False).execute()
        else:
            due_res = supabase.table("locations").select("party_name, current_due").order("party_name", desc=False).limit(10).execute()
        ...
        st.error(f"ডিউ তালিকা লোড করতে সমস্যা হয়েছে: {e}")

# 2. ROUTE MAP
elif selected_menu == "Route Map (রুট ম্যাপ)":
    st.write("### Route Map & Locations (রুট ম্যাপ)")
    try:
        route_res = supabase.table("locations").select("party_name, address, lat, lon, party_phone").not_.is_("lat", "null").not_.is_("lon", "null").order("party_name", desc=False).execute()
        route_data = route_res.data if route_res.data else []
    except Exception as e:
        route_data = []
        st.error(f"লোকেশন ডাটা আনতে সমস্যা: {e}")
...
# 3. ATTENDANCE MANAGEMENT
    ...
    try:
        ...
        agent_name_map = {r["username"]: (r.get("fullname") if r.get("fullname") else r["username"]) for r in att_users_data if r.get("username")}
    except Exception as e:
        att_users_data = []
        agent_name_map = {}
    ...
    with att_tab1:
        ...
        with st.form("attendance_form", clear_on_submit=True):
            ...
            if submit_att:
                try:
                    exist_att = supabase.table("attendance").select("id").eq("username", agent_for_att).eq("date", today_date_str).execute()
                    if exist_att.data:
                    ...
                    st.error(f"উপস্থিতি এনট্রি দিতে সমস্যা: {e}")

        st.markdown("---")
        st.markdown("#### Today's Attendance List (আজকের উপস্থিতি তালিকা)")
        
        try:
            today_att_res = supabase.table("attendance").select("date, username, in_time, status").eq("date", today_date_str).order("in_time", desc=True).execute()
            ...
            st.error(f"উপস্থিতি ডাটা আনতে সমস্যা: {e}")
        ...
        if current_role == "admin":
            ...
            if staff_list:
                ...
                if selected_rep_agent:
                    ...
                    try:
                        att_month_res = supabase.table("attendance").select("date").eq("username", selected_rep_agent).ilike("date", f"{month_prefix}%").execute()
                    ...
                    try:
                        ag_att_res = supabase.table("attendance").select("date, in_time, status").eq("username", selected_rep_agent).order("date", desc=True).execute()
            ...
            if del_mode == "Delete Single Date":
                ...
                if st.button("Delete Selected Attendance Record", type="primary", key="btn_del_att"):
                    if staff_list and del_agent:
                        ...
                        try:
                            rec_check = supabase.table("attendance").select("id").eq("username", del_agent).eq("date", del_date_str).execute()
                            if rec_check.data:
                ...
                if st.button("⚠️ Delete All Attendance for This Month", type="primary", key="btn_del_full_month"):
                    ...
                    try:
                        ...
                        st.error(f"বাল্ক ডিলিটে সমস্যা: {e}")
        else:
            st.write("#### Your Monthly Attendance Report")
            try:
                staff_att_res = supabase.table("attendance").select("date, in_time, status").eq("username", current_user).order("date", desc=True).execute()
        ...
        for idx, r in filtered_df.iterrows():
            ...
            with st.expander(f"Agent: {name} (ID: {username})", expanded=True):
                ...
                if pd.notna(lat) and pd.notna(lon):
                    ...
                    st.error("GPS coordinates not available for this agent.")
    else:
        st.warning("কোনো এজেন্টের লাইভ লোকেশন ডাটা পাওয়া যায়নি বা টেবিলটি খালি আছে।")

# 5. SETTINGS & AGENTS MANAGEMENT
                ...
                if new_uname.strip() and new_pass.strip() and new_fname.strip():
                    ...
                    if check_exist.data:
                        ...
                        try:
                            ...
                            st.error(f"এজেন্ট তৈরিতে সমস্যা: {e}")
                else:
    ...
    with set_tab4:
        ...
        st.caption("আপনার ডাটাবেসের ক্লাউড স্ট্যাটাস এবং রিয়েল-টাইম ক্লাউড সিঙ্ক তথ্য।")
        st.divider()
        col_backup, col_restore = st.columns(2, gap="large")
        
        with col_backup:
            st.markdown("#### Cloud Database Status")
            st.info("আপনার সমস্ত ডাটাবেস রিয়েল-টাইমে Supabase Cloud PostgreSQL-এ নিরাপদভাবে সংরক্ষিত হচ্ছে।")
            st.success("Supabase Realtime Sync Active")
            
        with col_restore:
            st.markdown("#### Cloud Backup Management")
            st.warning("Supabase ক্লাউড ডাটাবেস স্বয়ংক্রিয়ভাবে দৈনিক ব্যাকআপ গ্রহণ করে। কোনো ম্যানুয়াল রিস্টোর বা ডাটাবেস রিসেট প্রয়োজন হলে Supabase Dashboard ব্যবহার করুন।")
    ...
    with set_tab5:
        ...
        try:
            ...
            st.error(f"Error loading Recycle Bin: {e}. ডাটাবেসে recycle_bin টেবিলটি আছে কিনা চেক করুন।")
    ...
    with set_tab6:
        ...
        with st.form("update_admin_pass"):
            ...
            btn_update_pass = st.form_submit_button("Update Password", type="primary")
